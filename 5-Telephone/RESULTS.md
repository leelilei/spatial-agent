...
| 06-19 | P1-mech | DISSOCIATION MECHANISM (P4): heard-ratio probe | analysis of M4+G1 (0-API) | dose-response real (held rises with heard cur-frac, →95%) BUT mean ratio similar across baseline/source/broadcast (0.64/0.72/0.73) w/ opposite outcomes → naive exposure ratio REFUTED; received evidence under-determines HELD and motivates the matched-stream integration test C18 | done(tempered) |
...
| 07-23 | C18 | FIXED-STREAM listener-integration test: hold every per-agent received event fixed, replay GA vs PROV | 8 realized PROV dialogue streams (seeds 41-48), mini, 25a r5; same text+metadata into both memories; seed is independent unit | **PROV 60.5% [52.8-68.2] vs GA 32.0% [28.0-36.0]; paired lift +28.5pt [21.5-35.5], PROV higher 8/8 seeds (exact two-sided sign p=.0078).** Unknown falls 60.5%->34.5%. This makes listener integration a causal locus under matched received evidence; it does NOT prove integration is the only mechanism or that natural dialogue preserves provenance. | **done(n=8; reconstruction audited)** |
| 07-23 | C19 | GA-GENERATED cross-source replay: does the complete listener effect survive without PROV-generated dialogue support? | 8 realized one-shot GA dialogue streams (seeds 41-48), mini, 25a r5; identical text and controlled-task versions on unambiguous claims into GA/PROV listeners | **PROV 41.0% [28.1-53.9] vs GA 15.0% [7.9-22.1]; paired lift +26.0pt [16.5-35.5], positive in 8/8 seeds (exact two-sided sign p=.0078).** The effect survives GA-generated support, conditional on supplied versions. PROV stale also rises 8%->19% when only version-0 evidence arrives; net current-minus-stale CI crosses zero. | **done(n=8; checkpointed)** |
| 07-23 | C20 | SAID statistical-unit and agent-alignment audit | M4 baseline/source, five paired seeds; final-round utterances plus private interviews | Pooled SAID reproduces 62.9%->81.8%, but the paired mean of seed-specific shares is only **+7.8pt [-40.3,56.0]** because denominators are small. Among agents that said current, HELD-current is 10/14 baseline and 12/19 source. The defensible phenomenon is public-record visibility vs population-wide retrieval, not widespread within-agent forgetting. | **done; manuscript claim narrowed** |
| 07-24 | C21 | TABLE-COVERAGE pilot for missing architecture cells | mini, 25a m2 r5 t3, seed 41 only; {raw,mem0,memorybank} × {book_club,carpool,dues} | Current HELD: book club 8/56/44; carpool 36/12/4; numeric dues 8/20/28 (Raw/Mem0/MemoryBank, %). These are deliberately small **n=1 exploratory pilots**: they fill coverage cells but are excluded from rankings and inference; the large cross-scenario variation is itself a warning against treating them as stable estimates. | **done(n=1 pilot)** |
| 07-25 | C22 | POWER THE COVERAGE TABLE: replace every non-DeepSeek pilot with multi-seed estimates | mini, 25a m2 t3; standard tasks r5, three successive updates r7, long horizon r10; seeds 41--45 unless noted | **All non-DS pilot marks are removed.** Raw/Mem0/MemoryBank become n=5: book club 28/66/42, carpool 11/19/18, dues 16/22/27. Three-step n=5: Raw10, Mem0 19, A-MEM14, GA18, GA-current8, MemoryBank11, **PROV68 [59,77]**. Long horizon: Raw8, Mem0 30, A-MEM18, GA-current33, MemoryBank13, **PROV93 [82,100]**; the exact-matched GA comparator reuses the already powered C17 n=8 result **22 [15,28]** because `prov_mention` is not read by the GA factory. Wide intervals are retained rather than hidden. | **done(non-DS; DS deferred)** |
| 07-25 | C22-net | Provider/transport audit while completing C22 | FHL endpoint; curl/LibreSSL and urllib retries; failed jobs write no interview and are excluded | Six-way concurrency triggered TLS handshake failures; later the managed environment denied DNS. Switching from curl to urllib passed a live one-call probe and completed the two missing MemoryBank seeds. Failed/partial directories contain no final interviews and contribute zero rows. The last GA reruns were unnecessary after code/config audit established that C17 supplies an exact r10 GA n=8 comparator. | **closed(infrastructure only; no scientific cells)** |
| 07-25 | C23 | TABLE-1 SUMMARY METRIC audit | Re-analysis only; four matched mini/r5 tasks, equal-weight task macro-average | Removed `Calls/agent`: its 15.2/20.2 values mixed shared dialogue+interview calls with memory-update calls, were measured at r5 beside r7/r10 outcomes, and did not establish token/latency/cost efficiency. Replaced it with **Mean r5 Current HELD** across Repair drive, Book club, Carpool, and Numeric dues: Raw17.4, Mem0 31.4, A-MEM31.4, GA28.5, GA-current24.6, MemoryBank28.2, **PROV61.0**. The summary excludes r7/r10/DeepSeek and adds no new experiment. | **done(metric corrected)** |
| 07-25 | C24 | OFFICIAL-API DEEPSEEK TABLE COVERAGE | DeepSeek official API, `deepseek-v4-flash`, repair_drive, 25a m2 r5 t3, seeds 41--45; Raw/Mem0/A-MEM/GA-current/MemoryBank | Filled all five deferred DeepSeek cells: **Raw 39.2 [27.3,51.1]**, Mem0 26.4 [16.1,36.7], A-MEM 30.4 [12.3,48.5], GA-current 13.6 [5.3,21.9], MemoryBank 26.4 [15.0,37.8]. Existing powered same-model GA/PROV cells remain 15.5 [10.1,20.9] and **64.0 [57.8,70.2]** (n=8). PROV remains first; Raw becomes the second-highest point estimate in this column. All 10,725 logical calls succeeded without a transport retry across these five new methods. | **done(n=5; table complete)** |
