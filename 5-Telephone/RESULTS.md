...
| 06-26 | C17b | APM communication-sufficiency CURVE: held-current vs mention probability (sparsity sweep) | mini r10, apm K=1, 11 points (mention 0->0.8), n=3 (0.1 is n=8) | **Smooth S-curve, NO propagation threshold: 4%(m=0, only source knows) -> 20%(0.03) -> 27%(0.05) -> 37%(0.07) -> 40%(0.1) -> 65%(0.2) -> 81%(0.3) -> 100%(0.5,0.8). STALE==0 at EVERY point.** Truth spreads even at 3% mention (no hard threshold); 40% is the sparse-end honest floor; 100% only at the unrealistic dense limit (C5/C7). Safety (stale 0) is a FLAT line, independent of comms density. -> Fig 9 (rendered). | done |
