...
- DOI: none
- arXiv: 2303.17760
- PDF: `assets\papers\pdf\01_agent_societies\12_camel-communicative-agents-for-mind-exploration-of-llm-society.pdf`
...
Source PDF: D:\0-Research\5-Telephone\assets\papers\pdf\01_agent_societies\12_camel-communicative-agents-for-mind-exploration-of-llm-society.pdf

Extraction:
- backend: pdfplumber
- extracted_at_utc: 2026-06-20T12:40:07+00:00
- page_count: 77
- status: ok
- text_char_count: 206639
...
- Related Work (page 2)
- Methodology (page 3)
  - Role-playing Framework (page 3)
  - Inception Prompting (page 5)
- Experiments (page 7)
  - Role-Playing for AI Society (page 7)
- Evaluation (page 8)
  - Agent Evaluation (page 8)
  - GPT4 for ChatBot Evaluation (page 9)
  - HumanEval(+) (page 10)
- Conclusion (page 10)
- Acknowledgements (page 10)
- Cooperative Role-Playing: The Good Mind (page 11)
- Cooperative Role-Playing: The Bad Mind (page 18)
- Code Inception Prompting (page 23)
- Data Generation Prompts for Code (page 24)
- Meta Data (page 25)
- Math and Science Datasets Generation Details (page 26)
- Challenges (page 28)
- Agent Evaluation: Sample Tasks & Solutions (page 29)
  - Example 1: (page 30)
  - Example 2: (page 33)
- GPT4 for ChatBot Evaluation (Emergence of Knowledge): Sample Questions (page 35)
- Dataset Analysis (page 38)
- Check List Requirements (page 44)
  - Broader Impacts and Limitations: (page 44)
  - Training Details: (page 44)
  - Compute: (page 44)
  - Licenses: (page 44)
  - Human Subjects: (page 46)
- Examples from Datasets (page 47)
- Task Agents (page 50)
- Embodied Agent (page 52)
- Critic-In-The-Loop (page 54)
- On the Role Selection (page 56)
- Comparison with None LLaMA Based Models (page 67)
- Performance of CAMEL Models on OpenLLM (page 67)
- Comparison to Zero-CoT (page 67)
- Inception Prompt Ablation (page 68)
- Additional Related Work (page 70)
...
Human User sentiment analysis to
that can monitor
social media platform
Role Assignment: for positive or negati
comments about a
AI Assistant: particular stock, and
Python Programmer execute trades base
sentiment analysis
AI User: results
Stock Trader
Role Playing Sess
Figure 1: CAMEL Role-Playing Framework
having an idea they want to implement, e.g. de
involved in this task would be an AI assistant
agent who is a stock trader. The task is made m
to a well-defined task for the assistant to solve
the specified task, after which they collaborativ
instruction-following fashion to solve the specifi
Our proposed framework is a novel role-playi
agents. Specifically, we concentrate on task-orie
one AI user. After the multi-agent system receiv
human users, a task-specifier agent will provid
Afterwards, the AI assistant and AI user will c
multi-turn conversations until the AI user determ
giving instructions to the AI assistant and direct
other hand, the AI assistant is designed to follow
specific solutions. The whole role-playing fram
Human Input and Task Specifying. The role-p
selected roles by humans. As an example in Fi
a trading bot for the stock market. Humans m
idea can be realized. What is needed is only to
idea. For instance, a Python Programmer could
of developing a trading bot for the stock marke
specifier agent will brainstorm a specific task tha
to complete based on the input idea. An example
a trading bot with a sentiment analysis tool that
negative comments about a particular stock, and
The main motivation for introducing a task spe
a concrete task prompt for realizing the task w
non-domain expert. Therefore, the task specifie
for the idea implementation. Please note that, w
society and Code scenarios, we generate roles an
relying on human inputs. For our generated Mat
subtopics, and problems automatically by prom
AI Assistant-User Role Assignment. After the
user role will be assigned to the user agent and
specified task. In practice, a system message is
to the assistant system prompt/message by P
A
are passed to the agents before the conversation
regressive language models [82]. When the syste
...
AI User terminal and enter the following commands:
pip install tweepy
...
import pandas as pd
AI Assistant import numpy as np
import yfinance as yf
```
Next request.
ur role-playing setup starts with the human user
op a trading bot for the stock market. The roles
nt who is a python programmer and an AI user
specific using our task specifier agent, leading
oth AI user and AI assistant are provided with
communicate by chatting with each other in an
task.
approach for studying multiple communicative
d role-playing that involves one AI assistant and
a preliminary idea and the role assignment from
detailed description to make the idea specific.
erate on completing the specified task through
s the task is done. The AI user is responsible for
he conversation toward task completion. On the
e instructions from the AI user and respond with
rk is depicted in Figure 1.
ing session will be instantiated from an idea and
e 1, a human has a preliminary idea to develop
or may not have the knowledge about how the
gnate the potential roles that can implement the
llaborate with a Stock Trader to realize the idea
After the idea and roles are determined, the task
e AI Assistant role can help with the AI user role
a specified task in this scenario could be: develop
n monitor social media platforms for positive or
ecute trades based on sentiment analysis results.
er is that conversational agents usually require
might be challenging or time-consuming for a
gent serves as an enhanced imagination module
n studying our framework at a large scale for AI
eas automatically by prompting LLMs instead of
d Science datasets we generated problem topics,
g LLMs.
k specification, The AI assistant role and the AI
assistant agent correspondingly to complete the
sed to each agent declaring their role. We refer
d that of the user by P . The system messages
U
art. Let F and F denote two large-scale auto1 2
essage is passed to those models respectively, we

obtain A ← FPA and U ← FPU which are referred to as the assistant and user agents respectively.
1 2
...
instruction message obtained at time t by I and the assistant solution by S . The set of conversational
t t
messages obtained up until time t is denoted by Equation (1) shown below:
M = {(I , S ), ..., (I , S )} = {(I , S )}|t (1)
t 0 0 t t i i i=0
At the next time step, t + 1, the AI user U takes the historical conversation message set M and
t
provides a new instruction I , as shown in Equation (2). The produced instruction message I is
t+1 t+1
then passed, along with message set M , to the AI assistant A. The AI assistant will then respond
t
with a solution, denoted by S in Equation (3):
t+1
I = U(Mt) (2) St + 1 = A(Mt, It + 1) (3)
t+1
After obtaining the solution S to the instruction I , the message set is updated using Equation
t+1 t+1
(4) to obtain M :
t+1
M ← M ∪ (I , S ) (4)
...
prompt P , the assistant system prompt P , and the user system prompt P . As an example, we
T A U
...
assistant system prompt P and the AI user system prompt P are mostly symmetrical and include
A U
...
Assistant System Prompt: User System Prompt:
Never forget you are a Never forget you are a <USER_ROLE> and I am a <ASSISTANT_ROLE>.
<ASSISTANT_ROLE> and I am a Never flip roles! You will always instruct me.
<USER_ROLE>. Never flip roles! We share a common interest in collaborating to successfully
Never instruct me! complete a task.
We share a common interest in I must help you to complete the task.
collaborating to successfully Here is the task: <TASK>. Never forget our task!
complete a task. You must instruct me based on my expertise and your needs to
You must help me to complete the complete the task ONLY in the following two ways:
task.
Here is the task: <TASK>. Never 1. Instruct with a necessary input:
forget our task! Instruction: <YOUR_INSTRUCTION>
I must instruct you based on your Input: <YOUR_INPUT>
expertise and my needs to complete 2. Instruct without any input:
the task. Instruction: <YOUR_INSTRUCTION>
Input: None
I must give you one instruction at The "Instruction" describes a task or question. The paired
a time. "Input" provides further context or information for the
You must write a specific solution requested "Instruction".
that appropriately completes the
requested instruction. You must give me one instruction at a time.
You must decline my instruction I must write a response that appropriately completes the
honestly if you cannot perform requested instruction.
the instruction due to physical, I must decline your instruction honestly if I cannot perform
moral, legal reasons or your the instruction due to physical, moral, legal reasons or my
capability and explain the capability and explain the reasons.
reasons. You should instruct me not ask me questions.
Unless I say the task is Now you must start to instruct me using the two ways described
completed, you should always above.
start with: Do not add anything else other than your instruction and the
optional corresponding input!
Solution: <YOUR_SOLUTION> Keep giving me instructions and necessary inputs until you think
the task is completed.
<YOUR_SOLUTION> should be When the task is completed, you must only reply with a single
specific, and provide preferable word <CAMEL_TASK_DONE>.
implementations and examples for Never say <CAMEL_TASK_DONE> unless my responses have solved your
task-solving. task.
...
parts of the AI assistant system prompt P shown below:
A
...
For the AI user system prompt P , we strive to maintain as much symmetry as possible with respect
U
...
AI Society
Assistant Role Generation Prompt: User Role Generation Prompt:
You are a helpful assistant that can play many Please list <NUM_ROLES> most common and diverse
different roles. Now please list <NUM_ROLES> groups of internet users or occupations.
different roles that you can play with your Use singular form. No explanation.
expertise in diverse fields. Sort them by Sort them by alphabetical order. No explanation
alphabetical order. No explanation required. required.
...
Please reply with the specified task in <WORD_LIMIT> words or less. Do not add anything else.
Assistant System Prompt User System Prompt:
Never forget you are a Computer Programmer and Never forget you are a person working in <DOMAIN>
I am a person working in <DOMAIN>. Never flip and I am a Computer programmer. Never flip roles!
roles! Never instruct me! You will always instruct me.
We share a common interest in collaborating to We share a common interest in collaborating to
successfully complete a task. successfully complete a task.
You must help me to complete the task using I must help you to complete the task using
<LANGUAGE> programming language. <LANGUAGE> programming language.
Here is the task: <TASK>. Never forget our task! Here is the task: <TASK>. Never forget our task!
I must instruct you based on your expertise and You must instruct me based on my expertise and
my needs to complete the task. your needs to complete the task ONLY in the
following two ways:
...
Instruction: <YOUR_INSTRUCTION>
I must give you one instruction at a time. Input: None
You must write a specific solution that
appropriately completes the requested The "Instruction" describes a task or question.
instruction. The paired "Input" provides further context or
You must decline my instruction honestly if you information for the requested "Instruction".
cannot perform the instruction due to physical,
moral, legal reasons or your capability and You must give me one instruction at a time.
explain the reasons. I must write a response that appropriately
Do not add anything else other than your solution completes the requested instruction.
to my instruction. I must decline your instruction honestly if I
You are never supposed to ask me any questions cannot perform the instruction due to physical,
you only answer questions. moral, legal reasons or my capability and explain
You are never supposed to reply with a flake the reasons.
solution. Explain your solutions. You should instruct me not ask me questions.
Your solution must be declarative sentences and Now you must start to instruct me using the two
simple present tense. ways described above.
Unless I say the task is completed, you should Do not add anything else other than your
always start with: instruction and the optional corresponding
input!
Solution: <YOUR_SOLUTION> Keep giving me instructions and necessary inputs
until you think the task is completed.
<YOUR_SOLUTION> must contain <LANGUAGE> code When the task is completed, you must only reply
and should be specific and provide preferable with a single word <CAMEL_TASK_DONE>.
implementations and examples for task-solving. Never say <CAMEL_TASK_DONE> unless my responses
Always end <YOUR_SOLUTION> with: Next request. have solved your task.
...
Doctor Farmer
Editor Fashion designer
Engineer Filmmaker
Entrepreneur Gamer
Event Planner Graphic designer
Financial Advisor Homemaker
...
Physical Therapist Real estate agent
Programmer Salesperson
Project Manager Scientist
Psychologist Social media manager
Public Relations Specialist Software engineer
Real Estate Agent Student
Researcher Teacher
Sales Representative Technician
Scientist Travel agent
Social Media Manager Translator
Software Developer Truck driver
Teacher Tutor
Technical Writer Veterinarian
Translator Video editor
Travel Agent Virtual assistant
Video Editor Web developer
Virtual Assistant Writer
Web Developer Yoga instructor
Writer YouTuber
Zoologist Zoologist
Figure 6: Generated Meta Data. The meta data
50 assistant roles and 50 user role are generated
...
Java Accounting
Python Agriculture
JavaScript Anthropology
C# Architecture
PHP Art
C++ Biology
Ruby Business
Swift Chemistry
Objective-C Communications
SQL Computer Science
Go Criminal Justice
Kotlin Culinary Arts
TypeScript Dentistry
R Economics
MATLAB Education
Perl Engineering
Shell Environmental Science
Visual Basic Fashion
Assembly Film
Dart Finance
Geography
Geology
...
Theater
Urban Planning
erated by LLMs for AI Society and Code datasets.
AI Society. 20 programming languages and 50

F Math and Science Datasets Gener
Math Dataset. Our Math dataset consists of
follows:
1. We ask GPT4 to generate 25 math topi
2. We then ask GPT4 to generate 25 subto
topics.
3. For each (topic,subtopic) pair we gene
Science Dataset. The same recipe is used to
Physics problem-solution pairs, 20K Biology pr
solution pairs all generated and solved by GPT
and 25 subtopics for each topic, however, the n
problems yielding the 20K problem-solution p
This is summarized in Table 4.
Table 4: Math and Science
Dataset Number of problem-solut
Math Dataset 50K
Science 20K Physics pairs, 20K
Dataset pairs, 20K Chemistry pairs

on Details
problem-solution pairs which are generated as
s relevant to each of the previously generated 25
and solve 80 problems using GPT4.
erate the Science dataset which consists of 20K
em-solution pairs, and 20K Chemistry problemSimilar to Math dataset we generate 25 topics
ber of problems per (topic,subtopic) pair is 32
per subject (Physics, Biology, and Chemistry).
asets Generation Summary
pairs Generation Process
...
ology
1. We ask GPT4 to generate 25 topics for each subject (Physics, Biology, and Chemistry).
...
Role Flipping Infinite Conversation
User Message: Assistant Message:
Instruction: Please provide me with access to the It seems like you have already received the
designated Google Docs folder. necessary information to develop the system. Is
Input: The Google Docs folder is named there anything else you need assistance with?
"Translated Documents Review" and I will share it
with your email address. User Message:
No, thank you for your help. If you have any
Assistant Message: further questions or need any assistance in the
Instruction: I have received the access to the future, please don’t hesitate to reach out.
designated Google Docs folder. Please provide
me with the first document that needs to be Assistant Message:
reviewed. You’re welcome! It was my pleasure to assist
Input: The first document is named "Document 1 - you. Don’t hesitate to contact me if you need any
Translated from English to French". further assistance. Have a great day!
User Message: User Message:
I’m sorry, but as an AI language model, I do Thank you, you too!
not have access to the names or content of the
documents you are reviewing. If you could provide Assistant Message:
me with the name of the second document and You’re welcome! Goodbye!
its original language, I can provide you with
suggestions and guidance on grammar, punctuation, User Message:
spelling, and consistency of terminology. Goodbye!
Assistant Message: Assistant Message:
Instruction: Please provide the name of the Goodbye!
second document and its original language.
Input: The second document is named "Document User Message:
2 - Translated from Spanish to French" and its Goodbye!
original language is Spanish.
Assistant Message:
Assistant Repeats Instruction Goodbye!
User Message: User Message:
Great! Here’s the next instruction: Goodbye!
...
experience levels. Assistant Message:
It seems like we are stuck in a loop. Is there
Assistant Message:
Sure, here’s an exercise that can help actors anything else I can help you with?
improve their ability to take direction:
Instruction: Design an exercise that helps actors User Message:
improve their ability to take direction. No, thank you. Have a great day!
Input: The exercise should focus on helping
actors understand and execute a director’s Assistant Message:
vision, and should be suitable for actors of all You’re welcome! You too, have a great day!
experience levels.
User Message:
Flake Reply
Thank you, goodbye!
...
GERD is a condition in which stomach acid fl
such as heartburn, regurgitation, difficulty
hoarseness or sore throat. Symptoms may
over, and may also be worse at night or in th
complications such as esophagitis, strictures,
risk of esophageal cancer.
GPT Single Shot Solution (gpt-3.5-turb
The task involves creating medical illustratio
doctor. The illustrations should accurately d
digestive system for use in educational cont
strike a balance between artistic flair and acc
The duration of the scripted classroom discu

s back into the esophagus, causing symptoms
wallowing, chest pain, chronic cough, and
worse after eating, lying down, or bending
morning. If left untreated, GERD can lead to
d Barrett’s esophagus, which can increase the
f the digestive system in collaboration with a
t internal structures and disease states of the
The artist and doctor will work together to
cy in demonstrating related health anomalies.
n will also be taken into consideration.
...
• Simplify the expression (2x2 + 3x − 4) − (x2 − 2x + 1).
Science
...
Below we show sample solutions obtained by
review.
Sample Chatbot Answers
Question: What is the slope of the line pa
[Category: Math]
LLaMA-7B Solution:
The slope of the line passing through the poi
Our Final Model Solution:
To find the slope of the line passing through
formula:
m = (y − y )/(x − x )
2 1 2 1
where m is the slope, (x , y ) are the coo
1 1
...
m = 1
The slope of the line passing through the poi
Now, I will evaluate the answers provided by
Assistant 1’s Answer: The answer is incomp
final answer.
Assistant 2’s Answer: Assistant 2 correctly
points. The calculations were done correctly
Based on the evaluation, I would give the fo
Assistant 1: 1 (incomplete answer) Assist
accurate answer)
Assistant 2 performed better in this case.

MA-7B v.s. our model and the resulting GPT4
g through the points (3, 4) and (7, 8)?
(3, 4) and (7, 8) is:
points (3, 4) and (7, 8), we can use the slope
ates of the first point, and (x , y ) are the
2 2
(3, 4) and (7, 8) is 1.
points (3, 4) and (7, 8), we can use the slope
ates of the first point, and (x , y ) are the
2 2
(3, 4) and (7, 8) is 1.
ssistant 1 and Assistant 2:
as Assistant 1 did not provide any steps or a
d the slope formula and plugged in the given
d the final answer of 1 is accurate.
ing scores:
2: 10 (correct problem-solving steps and
...
thermostat, maintaining a constant core body temperature of approximately 98.6◦F (37◦C).
...
ShareGPT and Alpaca datasets (denoted CAMEL∗). Compared to the Vicuna13B and LLaMA13B
...
Model size Average ∆
(25 shots, acc_norm) (10 shots) (5 shots) (0 shot)
LLaMA 13B 56.2 80.9 47.7 39.5 56.1 -
CAMEL 13B 55.6 79.3 49.7 47.4 58.0 1.9
33B 61.3 84.7 58.5 42.3 61.7 5.6
LLaMA
...
Assistant System Prompt User System Prompt:
Never forget you are a <ASSISTANT_ROLE> and I Never forget you are a <USER_ROLE> and I am
am a <USER_ROLE>. Never flip roles! We share a a <ASSISTANT_ROLE>. Never flip roles! We
common interest in collaborating to successfully share a common interest in collaborating to
complete a task. Here is the task: <TASK>. Never successfully complete a task. Here is the task:
forget our task! <TASK>. Never forget our task! When the task is
...
Felix Hill, Alden Hung, Zachary Kenton, Jessica Landon, Timothy Lillicrap, Kory Mathewson, Sonˇa
...
Chen, Doug Fritz, Jaume Sanchez Elias, Richard Green, Sonˇa Mokrá, Nicholas Fernando, Boxi Wu,
...
[36] Amelia Glaese, Nat McAleese, Maja Tre˛bacz, John Aslanides, Vlad Firoiu, Timo Ewalds, Maribeth Rauh,
...
[62] Jonáš Kulhánek, Vojteˇch Hudecˇek, Tomáš Nekvinda, and Ondˇrej Dušek. Augpt: Auxiliary tasks and
