...
ABSTRACT
Humans are social beings; we pursue social goals in our daily interactions, which
...
complex social interactions between artificial agents and evaluate their social intelligence. In our environment, agents role-play and interact under a wide variety of scenarios; they coordinate, collaborate, exchange, and compete with each
...
To study dynamic and goal-driven social intelligence, we present SOTOPIA (Figure 1), an openended general-domain environment that situates social agents in diverse social scenarios. SOTOPIA
is interactive: in multi-turn simulated communication, agents can use verbal and non-verbal communication together with physical actions.1 It also has a diverse task space: the combination of automatically generated scenarios, goals, characters, relationships, and other agents’ policies creates a
...
To evaluate multi-faceted social interactions, we cannot only consider completing major social goals,
...
the following desiderata: (1) Realistic: this is to evaluate and understand artificial agents’ behavior under realistic scenarios; (2) Mixed utilities: human motives are often driven by both explicit
and implicit incentives, and the environment should be able to evaluate the agents’ performance on
multiple dimensions; (3) Open-ended: to support large-scale simulation and evaluation, the environment should be able to produce new tasks satisfying the previous two desiderata procedurally,
...
a scenario context, characters, and their social goals, providing the background of the interaction.
...
feelings, it might be better to express subjective emotion. Interaction partner’s policy also heavily
influences the optimal behaviors. Consider another task illustrated in Figure 1, “selling BMW Z3
for no less than $3,400 ”. If the buyer gives a high offer, the seller might want to exploit the buyer’s
...
relationship constraints; for example, a family relationship is required for a family dinner scenario,
3
...
context easier for (2), we consider five types of relationships: family, friend, romantic, acquaintance,
and stranger. Refer to Appendix B for the limitations of this approach and potential extensions.
...
patio. Another person is interested in this chair.” The private information is the social goals which
are only visible to the respective agents, e.g. “Your goal is to buy the chair for $80.” is only visible
to the buyer agent, while “Your goal is to sell the chair for $90.” is only visible to the seller agent.
However, the as mentioned above combination of scenarios and characters is not arbitrary, since
scenarios often imply constraints for the agents. We call this kind of constraint scenario constraints.
In this paper, we mainly consider relationship constraints which determines the types of relationships between the sampled characters. Similar to characters and relationships, scenarios, including
...
corresponding social goal. We will call these models and humans with characters and goals agents,
which take turns (in a round-robin fashion, i.e. Agent 1 acts first and then Agent 2 acts and so on)
to perform actions in an episode. At their own turn, the agent can choose to speak, use non-verbal
...
the following paragraphs, we itemize all seven dimensions in SOTOPIA, each with a score range2
...
Published as a conference paper at ICLR 2024
Believability (BEL) [0–10] focuses on the ex
natural, realistic, and aligned with the agents’ c
of human behavior (Park et al., 2023). Specifi
agent interacts with others in a natural and rea
agent align with their character traits e.g., perso
Knowledge (KNO) [0–10] captures the agent’s
dimension is motivated by the fact that curiosity
damental human trait (Reiss, 2004; Maslow, 19
What information the agent has gained through
has gained is new to them, and whether the info
Secret (SEC) [-10-0]3 measures the need for ag
intention private (Reiss, 2004). From a game the
of utility (Gilpin & Sandholm, 2006). Howeve
trust and thus improve relationships (Jaffe´ & Do
or secretive intention the participant wants to ke
Relationship (REL) [-5–5] captures the funda
longing (Maslow, 1943; Be´nabou & Tirole, 20
the participant has with the other agent(s) befo
interactions with others help preserve or enha
ascertain whether these interactions also impact
Social Rules (SOC) [-10–0] concerns norms, reg
differentiate between two types of social rules: s
prohibited actions and the potential for punishm
encompass normative social rules (e.g., it is con
Financial and Material Benefits (FIN) [-5–5] p
by classic game theory (Gilpin & Sandholm, 20
ity to be comprised of both short-term monetar
payoffs (e.g., job security, stock holdings, fundi
4 RESEARCH QUESTIONS AND EXPE
Given a diverse set of social scenarios, goals, an
is the first time that we could evaluate genera
systematic manner. In the next three sections, w
study these questions: (A) To which extent can w
judgment when it comes to evaluating agents’ so
among models (§6) and between models and hu
To study these questions, we create 40 agents
generation procedure in §2. For each scenario, w
constraints, resulting in a set of 450 tasks. For ea
by enumerating all model pairs. We also simula
and humans on a challenging subset SOTOPIA
Specifically, we consider the following model
GPT-4 (OpenAI, 2023), Llama-2-70b-chat (Tou
NLP Team, 2023). We set the temperature of t
and the temperature of the evaluator to 0 to en
version of the above models to help reproducibil
each turn, we prompt the language model with th
history to generate an action (see §2.2 for the po
the use of SOTOPIA to understand social intera
3For the SEC and SOC, there are only negative
considered as a baseline for the agents.
4as will be shown in §6 it is the best among mode
5We fix GPT-4 to be gpt-4-0613, and GPT-3.5 to

to which the agent’s behavior is perceived as
acter profile, thus simulating believable proxies
y, we consider the following criteria: 1. If the
c manner (naturalness). 2. If the actions of the
ity, values, etc. (consistency).
ility to actively acquire new information. This
., the desire to desire to know or learn, is a funSpecifically, we consider the following criteria:
interaction, whether the information the agent
tion the agent has gained is important to them.
(humans) to keep their secretive information or
perspective, leaking secrets often leads to a loss
evealing secrets can be a powerful tool to build
va, 2020). In this dimension, we ask what secret
and whether they keep it successfully.
ntal human need for social connection and be-
. In this dimension, we ask what relationship
he interaction, and then evaluate if the agents’
their personal relationships. Additionally, we
social status or the reputation of the agent.
tions, institutional arrangements, and rituals. We
al norms and legal rules. Legal rules encompass
t by institutionalized force, while social norms
red rude to speak loudly in a library).
ins to traditional economic utilities as addressed
Burns et al., 2017). We consider financial utilenefits (e.g., earnings) and long-term economic
opportunities).
MENTAL SETUP
haracters, we simulate agents’ interactions. This
oal-oriented social agents in an interactive and
will demonstrate how SOTOPIA can be used to
se GPT-4 (OpenAI, 2023) as a proxy for human
l interactions (§5)? (B) What are the differences
ns (§7) in their goal-oriented social intelligence?
relationships, and 90 scenarios following the
ample 5 pairs of characters based on the scenario
ask, we simulate the interaction between models
he interaction between GPT-4 (OpenAI, 2023)4
d (§7) due to the limitation of resources.
r comparison: GPT-3.5 (Ouyang et al., 2022),
n et al., 2023), and MPT-30b-chat (MosaicML
agents to 1 to encourage diversity of responses,
the stability of the evaluation. We use a fixed
5 To use these models as agents in SOTOPIA, at
cenario, the character to play, and the interaction
ble actions). In this paper, as we are focusing on
n, we use the prompt method for LLMs which is
es since keeping secrets and social rules should be
...
5.1 DATA COLLECTION PROCEDURE Majority of
GPT-4 scores are
within human scores
We randomly sample a subset of two hundred episodes σ = 2.15.
±
There are more GPT-4
from §4, and run a controlled study with a set of scores that are
pre-qualified workers from Amazon Mechanical Turk. higher than human
scores than lower.
They are given instructions about the meaning of each
(human score
dimension as mentioned in §3 and shown examples of > 74% GPT-4 score)
−
...
ratings. As each dimension of each agent is rated by several human annotators, we calculate a human
score by averaging the scores from multiple annotators. The agreement between human annotators
is moderate with a Randolph κ score of 0.503 (Randolph, 2005). GPT-4 is tasked with a similar job
as human annotators. We prompt GPT-4 to generate a structured output with an integer GPT-4 score
...
SEC 0.22∗∗ - that the majority (> 74%) of GPT-4 scores concentrate around the
KNO 0.33∗∗ 0.19 human scores within a standard deviation. It can also be seen that
SOC 0.33∗∗ 0.42∗∗ the white area on the left is larger than the one on the right, which
BEL 0.45∗∗ 0.27∗ means that GPT-4 is more likely to rate higher instead of lower than
REL 0.56∗∗ 0.49∗∗
...
The correlations show that when models are role-playing, the GPTTable 1: Pearson correlation co- 4 scores have significant and strong correlations with the humans’
efficients and p-values between scores on GOAL, FIN, and REL dimensions. However, when huGPT-4 evaluation and human mans are role-playing, the correlations drop significantly on all but
judgment on models’ and huone dimension (GOAL). This indicates that GPT-4 could evaluate
mans’ output among different disocial interactions on some dimensions and that it is better for evalmensions. Strong and signifiuating models compared to humans. In Appendix G.3, we compare
cant correlations are in blue. On
the average GPT-4 scores and the range of human scores for a sinGOAL and models’ output GPT-4
performs the best. gle dimension of an agent in an episode. We find that GPT-4 scores
are typically within human score ranges on most dimensions except for SOC and SEC, where GPT-4
...
6In a pilot study, other models are not able to provide a meaningful evaluation. See Appendix G.1.
7Including model-human, model-model, and human-human interaction.
8Without knowing whether it is a model or a human that role-plays a character.
6

Published as a conference paper at ICLR 2024
performance on the GOAL dimension. However, we remind readers that LLMs are known to have
...
GPT-3.5, Llama-2-70b-chat, and across different partner models. The best performance for each diMPT-30b-chat. mension is bolded when significantly better than the second best in
t-test (p < 0.05).
...
2-L
TPM
Weaker partners models weaken their conversation partners Fig3.32 2.70 2.42 1.76 ure 3, shows the overall performance of model pairs, which is the average performance across different dimensions. It is noticeable that a
...
have a negative score in the SOC and SEC dimensions. Even though GPT-4 performs better in most
...
9Presented are automated evaluation results. The human evaluation shows a similar trend, see Table G.3
10Some reported results could come from different versions of GPT-3.5.
7
...
GPT-4’s interactions with GPTGOAL BEL REL KNO SEC SOC FIN
4 and human annotators as the GPT-4 (w H) 4.85 9.25 0.70 2.80 0 0 0.50
evaluators. As shown in Table Human (w G) 5.95∗ 9.15 0.60 2.95 0 -0.60 0.70
3, humans perform significantly Human (w H) 6.15∗ 9.10 0.80 2.65 0 -0.10 0.45
better than GPT-4 in the GOAL
dimension. Table 3: Human and GPT-4 performance on different dimensions on
SOTOPIA-hard. SOC and SEC have the scale of -10 to 0, REL and FIN
It is also worth noting that hu- have the scale of -5 to 5, and others have the scale of 0 to 10. (w H) inmans on average produce 16.8 dicates that the agent is interacting with humans, while (w G) indicates
words per turn, while GPT-4 pro- that the agent is interacting with GPT-4. * indicates the difference is sigduces 45.5 words per turn, which nificant compared to GPT-4 (w H) with p < 0.05 under student’s t-test.
...
11See Appendix F for the detailed instructions and the chat interface.
8
...
et al., 2020; Baha et al., 2023; Dog˘ruo¨z & Skantze, 2021; Skantze & Dog˘ruo¨z, 2023). Such systems
...
persona and achieve explicit social goals spontaneously, which is more challenging than the existing
dialogue systems.
...
such as memorization, planning, and reflecting the past actions while ignoring other important dimensions such as SOC and SEC during social interactions. CAMEL Li et al. (2023a) simulates the
...
Published as a conference paper at ICLR 2024
REFERENCES
Tarek Ait Baha, Mohamed El Hajji, Youssef Es
alization: A systematic review of personality
2023. URL https://api.semanticscholar
Daniel S Bernstein, Robert Givan, Neil Immer
decentralized control of markov decision pro
819–840, 2002.
Federico Bianchi, Patrick John Chia, Mert Yukse
Zou. How well can llms negotiate? negotiatio
Tom Burns, Ewa Roszkowska, Ugo Corte, and
theory: Agency, social structures and interac
187–199, 01 2017. doi: 10.15290/ose.2017.0
Roland Be´nabou and Jean Tirole. Incentives an
96(5):1652–1678, December 2006. doi: 10
org/articles?id=10.1257/aer.96.5.165
Chi-Min Chan, Weize Chen, Yusheng Su, Jian
Zhiyuan Liu. Chateval: Towards better llm-b
...
in LLM simulations. In Houda Bouamor,
the 2023 Conference on Empirical Methods i
Singapore, December 2023b. Association for
emnlp-main.669. URL https://aclanthol
Jan Cieciuch and Eldad Davidov. A comparis
the pvq-21 to measure human values acros
Methods, 6(1):37–48, Apr. 2012. doi: 10.18
uni-konstanz.de/srm/article/view/509
Elisa De Stefani and Doriana De Marco. Lang
embodied view of social interaction. Front. P
Hiroshi Deguchi. Multi agent economics
ings Volumes, 28(7):269–274, 1995. IS
S1474-6670(17)47121-7. URL https://w
S1474667017471217. IFAC/IFIP/IFORS/SE
tional and Regional Economies 1995, Gold C
Michael Dennis, Natasha Jaques, Eugene Vinitsk
and Sergey Levine. Emergent complexity an
design. Advances in neural information proce
Ameet Deshpande, Tanmay Rajpurohit, Karthik
phization of ai: Opportunities and risks, 2023
A. Seza Dog˘ruo¨z and Gabriel Skantze. How “o
bots? a proposal for speech event based eva
ing of the Special Interest Group on Discour
line, July 2021. Association for Computation
2021.sigdial-1.41.
Radwan El Othman, Rola El Othman, Rabih H
traits, emotional intelligence and decision-ma
BMC psychology, 8:1–14, 2020.

ady, and Hammou Fadili. The power of personaptive chatbots. SN Computer Science, 4:1–25,
rg/CorpusID:261346287.
n, and Shlomo Zilberstein. The complexity of
es. Mathematics of operations research, 27(4):
onul, Jacopo Tagliabue, Dan Jurafsky, and James
ena platform and analysis, 2024.
ra Machado des Johansson. Sociological game
processes. Optimum. Studia Ekonomiczne, pp.
9.13.
rosocial behavior. American Economic Review,
7/aer.96.5.1652. URL https://www.aeaweb.
n Yu, Wei Xue, Shanghang Zhang, Jie Fu, and
evaluators through multi-agent debate, 2023.
rked personas: Using natural language prompts
Proceedings of the 61st Annual Meeting of the
me 1: Long Papers), July 2023a.
MPosT: Characterizing and evaluating caricature
n Pino, and Kalika Bali (eds.), Proceedings of
atural Language Processing, pp. 10853–10875,
mputational Linguistics. doi: 10.18653/v1/2023.
.org/2023.emnlp-main.669.
of the invariance properties of the pvq-40 and
erman and polish samples. Survey Research
/srm/2012.v6i1.5091. URL https://ojs.ub.
ge, gesture, and emotional communication: An
hol., 10:2063, September 2019.
d its gaming simulation. IFAC Proceed1474-6670. doi: https://doi.org/10.1016/
sciencedirect.com/science/article/pii/
Symposium on Modelling and Control of Na-
, Queensland, Australia, 2-5 July 1995.
Alexandre Bayen, Stuart Russell, Andrew Critch,
ero-shot transfer via unsupervised environment
ng systems, 33:13049–13061, 2020.
arasimhan, and Ashwin Kalyan. Anthropomor-
” are the conversations with open-domain chation. In Proceedings of the 22nd Annual Meetand Dialogue, pp. 392–402, Singapore and Oninguistics. URL https://aclanthology.org/
t, Sahar Obeid, and Souheil Hallit. Personality
g styles in lebanese universities medical students.
...
Weiyan Shi, Joe Spisak, Alexander Wei, David Wu, Hugh Zhang, and Markus Zijlstra. Humanlevel play in the game of ¡i¿diplomacy¡/i¿ by combining language models with strategic reasoning. Science, 378(6624):1067–1074, 2022. doi: 10.1126/science.ade9097. URL https:
...
des Sciences de l’Administration, 2(1):192–210, 1985.
Maxwell Forbes, Jena D. Hwang, Vered Shwartz, Maarten Sap, and Yejin Choi. Social chemistry
101: Learning to reason about social and moral norms. In Proceedings of the 2020 Conference on
Empirical Methods in Natural Language Processing (EMNLP), pp. 653–670, Online, November
...
abstraction and real-time equilibrium computation. In Proceedings of the 21st National Conference on Artificial Intelligence - Volume 2, AAAI’06, pp. 1007–1013. AAAI Press, 2006. ISBN
9781577352815.
Erving Goffman. The presentation of self in everyday life. Penguin Modern Classics. Penguin
Classics, London, England, May 1959.
Lewis R Goldberg. The development of markers for the Big-Five factor structure. Psychol. Assess.,
4(1):26–42, March 1992.
Jesse Graham, Brian A Nosek, Jonathan Haidt, Ravi Iyer, Spassena Koleva, and Peter H Ditto.
Mapping the moral domain. J. Pers. Soc. Psychol., 101(2):366–385, August 2011.
Katherine Hamilton, Shin-I Shih, and Susan Mohammed. The development and validation of the
rational and intuitive decision styles scale. J. Pers. Assess., 98(5):523–535, September 2016.
Elizabeth M. Minei Harry Weger, Gina Castle Bell and Melissa C. Robinson. The relative effectiveness of active listening in initial interactions. International Journal of Listening, 28(1):13–31,
2014.
...
v1/P17-1162. URL https://aclanthology.org/P17-1162.
...
Language Processing, pp. 2333–2343, Brussels, Belgium, October-November 2018. Association
...
choice models using Agent-Based modeling. Environment and planning. B, Planning & design,
41(4):661–689, August 2014. URL https://doi.org/10.1068/b120043p.
...
Mariela E Jaffe´ and Maria Douneva. Secretive and close? how sharing secrets may impact perceptions of distance. PLoS One, 15(6):e0233953, June 2020.
...
115(39):9696–9701, 2018. doi: 10.1073/pnas.1719452115. URL https://www.pnas.org/doi/
abs/10.1073/pnas.1719452115.
Hang Jiang, Xiajie Zhang, Xubo Cao, Cynthia Breazeal, Jad Kabbara, and Deb Roy. Personallm:
Investigating the ability of large language models to express personality traits. 2023. URL https:
...
experiment, 2022.
Katharina Kann, Abteen Ebrahimi, Joewie Koh, Shiran Dudy, and Alessandro Roncone. Opendomain dialogue generation: What we can do, cannot do, and should do next. In Proceedings of the 4th Workshop on NLP for Conversational AI, pp. 148–165, Dublin, Ireland, May
...
John F. Kihlstrom and Nancy Cantor. Social Intelligence, pp. 756–779. Cambridge Handbooks in
...
Grgur Kovacˇ, Re´my Portelas, Katja Hofmann, and Pierre-Yves Oudeyer. Socialai: Benchmarking
...
Processing (EMNLP-IJCNLP), pp. 5872–5877, Hong Kong, China, November 2019. Association
...
Park, Hancheng Cao, Tony Lee, Rishi Bommasani, Michael S. Bernstein, and Percy Liang. Evaluating human-language model interaction. Transactions on Machine Learning Research, 2023.
ISSN 2835-8856. URL https://openreview.net/forum?id=hjDYJUn9l1.
Mike Lewis, Denis Yarats, Yann Dauphin, Devi Parikh, and Dhruv Batra. Deal or no deal? endto-end learning of negotiation dialogues. In Proceedings of the 2017 Conference on Empirical
...
Camel: Communicative agents for” mind” exploration of large language model society. In Thirtyseventh Conference on Neural Information Processing Systems, 2023a.
...
acquisition with theory of mind. In The Eleventh International Conference on Learning Representations, 2022.
Qian Liu, Yihong Chen, B. Chen, Jian-Guang Lou, Zixuan Chen, Bin Zhou, and Dongmei Zhang.
You impress me: Dialogue generation via mutual persona perception. In Annual Meeting of the
Association for Computational Linguistics, 2020. URL https://api.semanticscholar.org/
CorpusID:215745354.
Ryan Liu, Howard Yen, Raja Marjieh, Thomas L. Griffiths, and Ranjay Krishna. Improving interpersonal communication by simulating audiences with language models. ArXiv, abs/2311.00687,
2023. URL https://api.semanticscholar.org/CorpusID:264832770.
...
A H Maslow. A theory of human motivation. Psychol. Rev., 50(4):370–396, July 1943.
...
Resources and Evaluation Conference, pp. 5731–5738, Marseille, France, June 2022. European
Language Resources Association. URL https://aclanthology.org/2022.lrec-1.616.
...
Published as a conference paper at ICLR 2024
The MosaicML NLP Team, 2023. URL https://www.mosaicml.com/blog/mpt-30b.
Ranjit Nair, Milind Tambe, Makoto Yokoo, David Pynadath, and Stacy Marsella. Taming decentralized pomdps: Towards efficient policy computation for multiagent settings. In IJCAI, volume 3,
pp. 705–711, 2003.
...
ISSN 1090-5138. doi: https://doi.org/10.1016/j.evolhumbehav.2023.06.002. URL https://www.
...
(UIST ’22), UIST ’22, New York, NY, USA, 2022. Association for Computing Machinery. ISBN
9781450393201. doi: 10.1145/3526113.3545616. URL https://doi.org/10.1145/3526113.
3545616.
...
Fixed-Marginal multirater kappa. In Proceedings of JLIS, 2005. URL https://eric.ed.gov/
?id=ED490661.
Sumedh Rasal. Llm harmony: Multi-agent communication for problem solving. ArXiv,
abs/2401.01312, 2024. URL https://api.semanticscholar.org/CorpusID:266725580.
...
Carolyn Rose´, Yi-Chia Wang, Yue Cui, Jaime Arguello, Karsten Stegmann, Armin Weinberger, and
Frank Fischer. Analyzing collaborative learning processes automatically: Exploiting the advances
of computational linguistics in computer-supported collaborative learning. International Journal
of Computer-Supported Collaborative Learning, 3(3):237–271, 2008. ISSN 1556-1615. doi:
10.1007/s11412-007-9034-0. URL https://doi.org/10.1007/s11412-007-9034-0.
Maarten Sap, Hannah Rashkin, Derek Chen, Ronan Le Bras, and Yejin Choi. Social IQa: Commonsense reasoning about social interactions. In Proceedings of the 2019 Conference on Empirical Methods in Natural Language Processing and the 9th International Joint Conference on
Natural Language Processing (EMNLP-IJCNLP), pp. 4463–4473, Hong Kong, China, November 2019. Association for Computational Linguistics. doi: 10.18653/v1/D19-1454. URL
...
2023.
Yunfan Shao, Linyang Li, Junqi Dai, and Xipeng Qiu. Character-LLM: A trainable agent for roleplaying. In Houda Bouamor, Juan Pino, and Kalika Bali (eds.), Proceedings of the 2023 Conference on Empirical Methods in Natural Language Processing, pp. 13153–13187, Singapore, December 2023. Association for Computational Linguistics. URL https://aclanthology.org/
2023.emnlp-main.814.
...
reasoning in large language models. arXiv, 2023a. URL https://arxiv.org/abs/2305.14763.
...
as the basis for human-like dialogue. In Proceedings of the 24th Meeting of the Special Interest
Group on Discourse and Dialogue, pp. 605–614, Prague, Czechia, September 2023. Association
for Computational Linguistics. URL https://aclanthology.org/2023.sigdial-1.57.
...
Language Processing (EMNLP), pp. 9275–9293, Online, November 2020. Association for Computational Linguistics. doi: 10.18653/v1/2020.emnlp-main.746. URL https://aclanthology.
org/2020.emnlp-main.746.
Leigh Tesfatsion and Kenneth L Judd. Handbook of Computational Economics: Agent-Based Computational Economics. Elsevier, May 2006. URL https://play.google.com/store/books/
...
Behavioral Sciences, 13(5):351, 2023.
...
John Agapiou, Jayd Matyas, and Dylan Hadfield-Menell. Melting pot contest, 2023. URL https:
...
v1/P19-1566. URL https://aclanthology.org/P19-1566.
...
1145/365153.365168. URL https://doi.org/10.1145/365153.365168.
...
18653/v1/2022.naacl-main.341. URL https://aclanthology.org/2022.naacl-main.341.
...
Ghanem, and G. Li. Can large language model agents simulate human trust behaviors? ArXiv,
abs/2402.04559, 2024. URL https://api.semanticscholar.org/CorpusID:267523076.
...
preprint arXiv:2308.04030, 2023.
...
A question answering benchmark for artificial social intelligence. In 2019 IEEE/CVF Conference
...
IEEE/CVF Conference on Computer Vision and Pattern Recognition, pp. 8807–8817, 2019b.
16
...
Saizheng Zhang, Emily Dinan, Jack Urbanek, Arthur Szlam, Douwe Kiela, and Jason Weston. Personalizing dialogue agents: I have a dog, do you have pets too? arXiv preprint arXiv:1801.07243,
2018b.
...
automated debiasing for toxic language detection. In EACL, 2021.
...
Caleb Ziems, Jane Dwivedi-Yu, Yi-Chia Wang, Alon Halevy, and Diyi Yang. NormBank: A knowledge bank of situational social norms. In Proceedings of the 61st Annual Meeting of the Association for Computational Linguistics (Volume 1: Long Papers), pp. 7756–7776, Toronto, Canada,
...
F The procedure and interface for humans13 when playing characters in SOTOPIA;
...
(Sawyer, 2005; Rose´ et al., 2008; Deguchi, 1995). Prior simulation environments have played a pivotal role in constructing theories and generating hypotheses in these fields. However, they frequently
...
turn-taking interaction to the dyadic context, studying interactions between two agents. Future works
...
13All the human subjects experiments are approved by the Institutional Review Board (IRB) at the authors’
institution.
18
...
SOTOPIA under the AI2 impact license14 to prevent misuse. Future work could further investigate
...
the case where the agents take turns to act in round-robin order, i.e. agent i only act at time step t
when t i mod N . For a long enough horizon, this generalizes to any conversation with proper
≡
turn-taking. In our experiments, we only consider N = 2 cases, while the environment is designed
to support any N 2 cases.
≥
14https://allenai.org/impact-license
19

Published as a conference paper at ICLR 2024
C.2 TASK SPACE TECHNICAL DETAILS
C.2.1 CHARACTERS
...
“agreeableness” and “neuroticism” (Goldberg, 1992). The moral value types are “care”, “fairness”,
“loyalty”, “authority” and “purity” (Cieciuch & Davidov, 2012). The Schwartz personal value types
are “self-direction”, “simulation”, “hedonism”, “achievement”, “power”, “security”, “conformity”,
“tradition”, “benevolence”, and “universalism” Cieciuch & Davidov (2012). The decision-making
style types are “directive”, “analytical”, “conceptual”, and “behavioral”. As previously studied in
...
scale and calculate the free-marginal multi-rate κ score.
...
• Soial Context: “A concrete scenario of where the social interaction takes place, the scenario
...
Note: (1) The available relationship types are: stranger, acquaintance, friend, romantic relationship,
...
G.2 PROVIDING EVALUATOR WITH FINE-GRAINED DESCRIPTION
...
GPT-4’s prediction is generally within the human perceived ranges. While for SEC and SOC, GPT4’s prediction is overly optimistic. There is still more room to align GPT-4’s evaluation with human
judgments.
G.4 MODEL PERFORMANCE IN SOTOPIA
...
in SOTOPIA-hard evaluated by human annotators.
29
...
Figure G.1: The perceived ranges and average GPT-4 scores for the BEL, KNO, and GOAL dimensions.
4.00 4.50
3.36
2.20 4.40
...
Figure G.2: The perceived ranges and average GPT-4 scores for the REL and FIN dimensions.
...
-9.50 -6.50
-10.00 -10.00
critical secrets non-critical secrets
law breaking morally wrong
Figure G.3: The perceived ranges and averag
Dim. GPT-4
SOC -0.36
SEC -0.27
FIN 0.42
...
BEL 7.63
Overall 2.81
Table G.3: The aggregated performance of each mod
paired with, evaluated by human annotators. The ov
sions. The best performance for each dimension is bo
BEL REL K
GPT-4 (w H) 8.48 0.65 1
Human (w G) 8.53 0.78 1
Human (w H) 8.43 0.93 2
Table G.4: Human and GPT-4 performance on diffe
annotators. SOC and SEC have the scale of -10 to 0, R
scale of 0 to 10. (w H) indicates that the agent is inte
is interacting with GPT-4. * indicates the difference
...
0.00
-1.00
minor information no secrets leaked
morally unacceptable morally acceptable
PT-4 scores for the SEC and SOC dimensions.
PT-3.5 Llama-2
-0.59 -0.67
-0.18 -0.37
0.27 0.12
...
6.80 4.28
2.18 1.48
y averaging across different reference models it gets
score is the average performance across all 7 dimend when significant.
SEC SOC FIN GOAL
0.00 -0.38 0.63 5.25
0.00 -0.70 0.75 6.53∗
-0.50 -0.45 0.33 6.05
dimensions on SOTOPIA-hard evaluated by human
and FIN have the scale of -5 to 5, and others have the
ng with humans, while (w G) indicates that the agent
ignificant compared to GPT-4 (w H) with p < 0.05
...
indicates the reference model. SOC and SEC have the scale of -10 to 0, REL and FIN have the scale of -5 to 5,
