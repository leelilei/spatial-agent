...
publisher_url: https://www.semanticscholar.org/paper/0671fd553dd670a4e820553a974bc48040ba0819
quality_flags: []
---
...
- Telephone index: 44
- Preferred source: Neural Information Processing Systems
- DOI: 10.52202/075280-0377
- arXiv: 2303.11366
...
Source PDF: D:\0-Research\5-Telephone\assets\papers\pdf\04_memory_state_agents\44_reflexion.pdf

Extraction:
- backend: pdfplumber
- extracted_at_utc: 2026-06-20T12:43:45+00:00
- page_count: 19
- status: ok
- text_char_count: 69659
...
- Introduction (page 1)
- Related work (page 2)
- Reflexion: reinforcement via verbal reflection (page 3)
- Experiments (page 5)
  - Sequential decision making: ALFWorld (page 5)
  - Reasoning: HotpotQA (page 6)
  - Programming (page 7)
- Limitations (page 9)
- Broader impact (page 9)
- Conclusion (page 9)
- Reproducibility (page 9)
- Evaluation with additional models (page 12)
- Decision-making (page 13)
  - WebShop Limitation (page 14)
- Programming (page 14)
  - Programming function implementation example (HumanEval Python) (page 14)
  - Reflexion Actor instruction (page 15)
  - Reflexion Self-reflection instruction and example (page 15)
  - Reflexion programming no Self-Reflection ablation example (page 15)
  - Reflexion programming no test generation ablation example (page 15)
- Reasoning (page 17)
  - Full example (page 17)
  - Chain-of-Thought + Reflexion (page 18)
  - HotPotQA Chain-of-Thought (GT) + Reflexion (page 18)
  - HotPotQA episodic memory (EPM) ablation prompts (page 19)
    - (EPM) Chain-of-Thought + Reflexion (page 19)
    - (EPM) Chain-of-Thought (GT) + Reflexion (page 19)
...
M , which generates text and actions; an Evaluator model, represented by M , that scores the outputs
a e
produced by M ; and a Self-Reflection model, denoted as M , which generates verbal reinforcement
a sr
...
Figure 2: (a) Diagram of Reflexion
Actor The Actor is built upon a large langu
generate the necessary text and actions condition
policy-based RL setups, we sample an action or
receive an observation from the environment o .
t
Thought [26] and ReAct [30]. These diverse gen
of text and action generation within the Reflexio
performance and effectiveness. In addition, we
additional context to this agent. This adaption wa
iteration approach using in-context learning. De
Evaluator The Evaluator component of the R
the quality of the generated outputs produced b
and computes a reward score that reflects its pe
effective value and reward functions that appl
several variants of the Evaluator model. For re
on exact match (EM) grading, ensuring that the
solution. In decision-making tasks, we employ
specific evaluation criteria. Additionally, we ex
LLM itself as an Evaluator, generating rewards
multi-faceted approach to Evaluator design all
generated outputs, offering insights into their ef
Self-reflection The Self-Reflection model in
Reflexion framework by generating verbal self
trials. Given a sparse reward signal, such as a bina
and its persistent memory mem, the self-reflecti
This feedback, which is more informative than
(mem). For instance, in a multi-step decision-ma
can infer that a specific action a led to subsequ
i
then verbally state that it should have taken a dif
and a′ , and store this experience in its memory
i+2
experiences to adapt its decision-making appro
process of trial, error, self-reflection, and persist
decision-making ability in various environments
Memory Core components of the Reflexion
memory. At inference time, the Actor conditions

Algorithm 1 Reinforcement via self-reflection
Initialize Actor, Evaluator, Self-Reflection:
M , M , M
a e sr
Initialize policy π (a |s ), θ = {M , mem}
θ i i a
Generate initial trajectory using π
θ
Evaluate τ using M
0 e
Generate initial self-reflection sr using M
0 sr
Set mem ← [sr ]
0
Set t = 0
while M not pass or t < max trials do
e
Generate τ = [a , o , . . . a , o ] using π
t 0 0 i i θ
Evaluate τ using M
t e
Generate self-reflection sr using M
t sr
Append sr to mem
t
Increment t
...
return
Reflexion reinforcement algorithm
model (LLM) that is specifically prompted to
n the state observations. Analogous to traditional
neration, a , from the current policy π at time t,
t θ
explore various Actor models, including Chain of
tion models allow us to explore different aspects
amework, providing valuable insights into their
o add a memory component mem that provides
spired by Brooks et al. [3], who suggest a policy
s on how this is populated are provided below.
xion framework plays a crucial role in assessing
e Actor. It takes as input a generated trajectory
rmance within the given task context. Defining
semantic spaces is difficult, so we investigate
ning tasks, we explore reward functions based
nerated output aligns closely with the expected
-defined heuristic functions that are tailored to
iment with using a different instantiation of an
decision-making and programming tasks. This
s us to examine different strategies for scoring
iveness and suitability across a range of tasks.
ntiated as an LLM, plays a crucial role in the
ections to provide valuable feedback for future
uccess status (success/fail), the current trajectory,
model generates nuanced and specific feedback.
ar rewards, is then stored in the agent’s memory
g task, when the agent receives a failure signal, it
incorrect actions a and a . The agent can
i+1 i+2
nt action, a′ , which would have resulted in a′
i i+1
subsequent trials, the agent can leverage its past
at time t by choosing action a′ . This iterative
i
memory enables the agent to rapidly improve its
utilizing informative feedback signals.
cess are the notion of short-term and long-term
decisions on short and long-term memory, similar
...
first trial, the Actor produces a trajectory τ by interacting with the environment. The Evaluator then
0
produces a score r which is computed as r = M (τ ). r is only a scalar reward for trial t that
0 t e 0 t
improves as task-specific performance increases. After the first trial, to amplify r to a feedback form
0
that can be used for improvement by an LLM, the Self-Reflection model analyzes the set of {τ , r }
0 0
to produce a summary sr which is stored in the memory mem. sr is a verbal experience feedback
0 t
for trial t. The Actor, Evaluator, and Self-Reflection models work together through trials in a loop
until the Evaluator deems τ to be correct. As mentioned in 3, the memory component of Reflexion
t
is crucial to its effectiveness. After each trial t, sr , is appended mem. In practice, we bound mem
t
...
Q → A and Q, C → A implementations, where Q is the question, C is the ground truth context
gt gt
...
to produce a test suite T , denoted as {t , t , . . . , t }. We set n to a maximum of 6 unit tests. Aside
0 1 n
...
Reflection: I searched the wrong title for the s
should have searched the show’s main character,
in the show.
Figure 7: Two HotPotQA trials within the same
uses self-reflection to determine a better search
...
w, "’Allo ’Allo!", which resulted in no results. I
den Kaye, to find the role he was best known for
ronment and task. The Reflexion + ReAct agent
hod for the next trial.
...
Reflection: My reasoning for the profession Joh
failed because I incorrectly assumed that they
attempting this question, I should focus on bet
authors to ensure that I am accurately identifyin
the possibility that they may have multiple profe
D.3 HotPotQA Chain-of-Thought (GT) + R
Context: The Battle of White Plains was a batt
American Revolutionary War fought on Octobe
the retreat ...
...
Observation 1: Answer is INCORRECT
Reflection: Upon reflecting on the incorrect ans
enough context to accurately answer the questi
only provided the name of one battle. To better a
provide more context, such as the name of the c
campaign, and the fact that it was a series of b
dates of the battles, as well as the locations, to en
...
anchester and Alan Dean Foster have in common
h had the same profession. In the future, when
esearching the individual backgrounds of both
heir professions. Additionally, I should consider
ons in common.
xion
n the New York and New Jersey campaign of the
, 1776, near White Plains, New York. Following
...
campaign]
Observation 1: Answer is CORRECT
I provided, I realize that I may not have provided
The question asked for a series of battles, but I
er this question in the future, I will make sure to
paign, which was the New York and New Jersey
s. Additionally, I will make sure to include the
e that my answer is comprehensive and accurate.

D.4 HotPotQA episodic memory (EPM) abl
...
Observation 1: Answer is INCORRECT
Reflection: My reasoning for which musician h
not take into account that Jonny Craig has been a
Doherty is currently a member of more bands. I
focus on researching the past and current band
comparing their band memberships.
D.4.2 (EPM) Chain-of-Thought (GT) + Refl
Context: Hari Bahadur Basnet is a Nepalese
Department of the Rastriya Janashakti Party. Ba
A Master of Science (Latin: "Magister Scientiae"
ScM, Sc.M., SciM or Sci.M.) is a master’s degr
...
Medicine]
Observation 1: Answer is INCORRECT
Reflection: For the first trial, I misunderstood t
category of degrees, rather than a specific degree
focused more closely on the question to underst
...
een a member of more bands failed because I did
mber of more bands in the past, even though Pete
e future, when attempting this question, I should
both musicians to ensure that I am accurately
on
tician. He is the head of the Foreign Relations
holds a M.Sc. in Engineering.
bbreviated MS, M.S., MSc, M.Sc., MSci, M.Sci.,
...
Observation 1: Answer is CORRECT
question. I thought the question was asking for a
earned from my mistake and in the second trial, I
that it was asking for a specific field of study.
