---
telephone_index: 23
title: "PRD: Peer Rank and Discussion Improve LLM based Evaluations"
...
- DOI: none
- arXiv: 2307.02762
- PDF: `assets\papers\pdf\02_debate_consensus\23_prd-peer-rank-and-discussion-improve-llm-based-evaluations.pdf`
...
Source PDF: D:\0-Research\5-Telephone\assets\papers\pdf\02_debate_consensus\23_prd-peer-rank-and-discussion-improve-llm-based-evaluations.pdf

Extraction:
- backend: pdfplumber
- extracted_at_utc: 2026-06-20T12:41:28+00:00
- page_count: 37
- status: ok
- text_char_count: 103656
...
- Methodologies (page 3)
  - Peer Rank and Scoring (PR) (page 4)
    - Win rate Calculation (page 4)
    - Elo Calculation (page 5)
  - Peer Discussions (PD) (page 5)
- Experiments and Analysis (page 7)
  - Datasets, Metrics, and Setup (page 7)
    - Datasets (page 7)
    - Metrics (page 8)
    - Setup (page 9)
  - Results for Peer Rank (PR) (page 9)
  - Results for Peer Discussions (PD) (page 10)
- Further Analysis (page 12)
- Conclusion (page 13)
- Proof of convergence of peer rank (page 18)
- LLM details (page 18)
- Major differences between our PR work and walsh2014peerrank (page 18)
- Major Differences between Self-Discuss and Self-Refine (page 19)
- Detailed Win rate & Elo Calculation (page 19)
- Term Definitions (page 20)
- Role Prompt (page 20)
- Pairwise win rate heatmap (page 21)
- Human Annotation for Pairwise Preference (page 22)
- Discussion Examples (page 24)
...
(LLMs) is hard to evaluate and compare automatically. Recent studies suggest and predominantly use LLMs for reference-free evaluation of open-ended question answering. More
...
domain (Cho & MacArthur, 2011; Walsh, 2014) to improve LLM-based evaluations. Specifically, we propose (1) the peer rank (PR) algorithm that takes into account each peer LLM’s
...
1Codes are available at: https://github.com/bcdnlp/PRD.
1
4202
ceD
13
...
Published in Transactions on Machine Learning Res
Motivated by these limitations, we propose the idea
automated evaluations while still benefiting from LLM
propose Peer Rank and Discussion-based evaluation
that share the same format and goal – involving p
fair evaluation result where all peers mutually ag
psychology research field on methodologies of studen
and benefits (Cho & MacArthur, 2011; Yalch et al.,
global rankings and induces reviewer weights. It wor
each LLM in pairwise matches produces an answer f
vote to decide the final preference scoring, we propo
Peer Discussion (PD) facilitates fine-grained pairwis
comparison setting. Given two candidate answers, w
discussions to reach a mutual agreement on the pair
format of LLM interacting with each other through
2023; Park et al., 2023; Fu et al., 2023b). PR and PD
of providing a more fair (de-biased) ranking of longexperiments and analysis for measuring PR and PD’
is tested on Vicuna80 Chiang et al. (2023), which co
method improves correlations with human ranking
LLMs to induce a self-ranking. PD is tested on both
annotated pairwise comparisons of Human-Machin
achieve better pairwise comparisons that are more a
PD mitigate the above mention biases especially se
more analysis for PD, showing: (1) the LLM leading
LLMs are more likely to hold their opinions.
2 Related Work
Automatic Evaluations Natural Language Ge
similarity-based or reference-free type. For similari
reference texts Papineni et al. (2002); Zhang et al. (20
metrics such as consistency (Kryściński et al., 2020;
et al., 2023) and coherence (Dziri et al., 2019). This
specific prompts for large language model-based eva
and can act as a unified evaluator (Zhong et al., 202
Specifically, for long-form or open-ended questio
measure the similarity between human and machine-ge
is not a fair metric for quality measurement due to
et al., 2021; Xu et al., 2023). Fu et al. (2023a) pro
pre-training models like GPT-3. Xu et al. (2023) imp
Given a prompt consisting of a question with two an
answer 1 or answer 2 (pairwise comparisons).
LLMs as evaluators: problems and challenges
evaluations for benchmarking the progress have bec
are tested on both standard datasets such as MMLU
are much more prevalent in real life (Nakano et al., 2
et al., 2023; OpenAI, 2023) as an evaluator for eit
et al., 2023b; Zhou et al., 2023). However, such a
biases, such as (1) positional bias (Dettmers et al., 2
answer in pairwise comparisons; (2) verbosity and len
self-enhancement bias, where an LLM favors its own

ch (07/2024)
peer evaluation. The goal is to mitigate the biases in
strong capability in reading and writing reviews. We
mework (PRD). The suit consists of two alternatives
LLMs’ participation as reviewers to reach a more
. We draw insights and lessons from educational
eer reviewing (Walsh, 2014), as well as their impact
9). More specifically, Peer Rank (PR) is utilized for
or the tournament-style benchmarking setting where
n open-ended question. Instead of using the average
weighted votes based on LLMs reviewers’ capabilities.
omparison/ranking. It works for the general pairwise
prompt two other reviewer LLMs to have multi-turn
e scoring or preference. The process shares a similar
nversations like two communicative agents (Li et al.,
re closely interrelated and fall under the same theme
free-form answers. enumerate We conduct extensive
pabilities of providing fair pairwise comparisons. PR
ins pairwise judgments from human annotators. Our
bstantially. This paradigm also enables a group of
cuna80 and LFQA (Xu et al., 2023), which includes
nd Machine-Machine answers. PD enables LLMs to
rate than single model-based reviews. Both PR and
nhancement bias significantly. Further, we provide
cussions is less likely to alter its opinion; (2) stronger
ation (NLG) evaluation methods are mainly of a
based metrics, the generated texts are compared to
). In parallel, people have also developed task-specific
ng et al., 2020), faithfulness (Fabbri et al., 2022; Gao
imilar to our peer discussion idea on designing more
tions. Our prompting-based method is more flexible
answering, early work uses ROUGE (Lin (2004)) to
ated answers. However, researchers find that ROUGE
e open-ended nature of long-form answers (Krishna
e GPTScore, which evaluates texts with generative
ments a similar idea for evaluating long-form answers.
r candidates, GPT-3 is fine-tuned to output the label
ost recently, with the trend of developing more LLMs,
e even more important but also more difficult. They
nd more importantly, on open-ended questions which
; Chiang et al., 2023). People mostly use GPT-4 (Liu
generating scores or pairwise comparisons (Wang
tegy has fundamental problems because of various
; Wang et al., 2023a), where a model favors the first
bias (Wang et al., 2023b); (3) and most importantly,
swers (Liu et al., 2023; Zheng et al., 2023).

Pairwise
Contestants
Battles Win Rate Matrix
Published in Transactions on Machine Learning Res
Reviewers
A B C
1
...
Contestants
Battles Win Rate Matrix
Figure 1: The peer rank process (PR): each LLM mo
2, 3). From the battles between contestants (pairwise
Reviewers
models A, B, and C represent GPT-4, BAard, andBCl
Efforts have been proposed to tackle them: (1) Usin
1
positional bias; (2) Zheng et al. (2023) proposes Cha
pairwise judgments of answers generated by two L
annotation to ensure fairness; (3) Bai et al. (2023)
2
generates questions to test other models. Differen
generated questions. Moreover, none of the above wo
Overall, our work, peer ev3aluation-oriented metho
variety of tasks, and focuses more on the alignment
Contestent
Pairwise Battles
LLM Interactions and Discussion WTinh Reartee Mhaatrvix
LLMs, including Liang et al. (2023); Du et al. (202
are task-oriented frameworks aiming at improving
focused on math tasks and question answering tasks.
translation and question answering. Chen et al. (2
implemented a multi-agent debate framework with m
interactions to accomplish tasks and improve mode
humans and another from LLM, our approach utilize
order to achieve better evaluation that align with hu
Peer Evaluation in the Educational Domain
human-in-the-loop studies, such as in the classroom
conduct human-oriented data collection and experi
2014). In contrast, we focus on automatic evaluatio
comparisons of LLMs’ answers. Moreover, our peer r
absolute grades.
3 Methodologies
In general, Peer Rank (PR) can be applied to ind
capabilities. Peer Discussion (PD) provides a more
answers. Both of them aim at reducing the bias in
...
Weight Vector Score Vector
acts both as reviewers (A, B, C) and contestants (1,
mparisons), it induces a self-ranking. In this example,
eC, respectively.
Better contestant weigh more
Better contestants
osition switching s(coWre haignhegr et al., 2023a) for mitigating
ot Arena, where real users ask questions and provide
s. But this is time-consuming and requires expert
opose using each LLM as an examiner, where each
rom PD, their “exams” are biased with randomly
support inducing self-rankings through peer ranking.
dot
product
undergo tests on two benchmarks, each covering a
ween LLMs’ evaluations and human judgments.
Normalize
een works that explore Multi-agent discussion with
Reviewer (Multi-round) Contestent
Chan et al. (2023); Chen et al. (2023). All of them
Weight Vector Score Vector
Ms’ performance on general tasks. Du et al. (2023)
ang et al. (2023) tested two tasks, including machine
3) covered math and reasoning. Chan et al. (2023)
iple prompts for evaluation. Prior works utilize LLM
accuracy. For our case, based on one response from
LM interactions to discuss which one is better and in
n preferences.
rior works on educational research mainly focus on
Cho & MacArthur, 2011; Nicol et al., 2014). They
nts to verify the benefits of peer evaluations (Walsh,
employing peer LLM reviewers to conduct pairwise
k process focuses on pairwise comparisons, instead of
self-ranking – a ranking of a group of LLMs’ own
e-grained and interactive comparison of two models’
tomatic evaluations. We elaborate on the technical
...
for the next round.
Formally, let W c be the raw win rate of contestant c ∈ C from the reviews of reviewer r ∈ R. This is equal to
r
...
X
scorek = αk−1 · W c, αk = Norm(MinMax(scorek))
c r r (2)
r∈R
...
2We provide the proof of convergence of our peer rank algorithm in Appendix A.
5
...
Firstly, please compare the two answers based on if they contain unsupported information, core information, and coherence. Please provide a comprehensive explanation of your
...
Published in Transactions on Machine Learning Res
[System] You are reviewer A, discussing with
answers.
[Question] {Q}
[Answer1] {A1} [Answer2] {A2}
[Init Review A] {Review of reviewer A} [Init
[System] "Read the reviews and discussions ab
preference, and explain. Remember we focus o
and coherence. In a new line, choose betwee
1 or 2 respectively. Do not output anything
[Reviewer A] {First-turn output}
[Reviewer B] {Second-turn output}
[Reviewer A]:
Table 2: The discussion template for reviewer A a
explicitly indicate aspects that reviewers need to p
are input to reviewer A LLM model.
discussion, reviewer A’s initial review is listed first
prompt specifies the detailed requirements, which p
Specifically, we draw insights from WebGPT (Naka
For long-form question answering, we focus on (1) U
support, assume the worst case: that all of it is false.
overall rating; (2) Core information: about whether
generally, it is less important than the two above. Th
we do not adjust the above prompts based on any d
4 Experiments and Analysis
4.1 Datasets, Metrics, and Setup
4.1.1 Datasets
We select two “meta-evaluation” datasets, LFQA (X
for pairwise comparisons, to measure the correlation
LFQA (Xu et al., 2023) contains 140 long-form qu
and biology) and two candidate answers (from eit
et al., 2019), it contains more recent (i.e., after July 2
and “r/AskHistorians”. The authors collected expe
preference).
Vicuna80 (Chiang et al., 2023) is a set of 80 open
a wide range of tasks, including question answerin
work (Dettmers et al., 2023), authors annotated p
models for each question. The scores include 0, 1, 2,
respectively. We select pairwise comparison annota
PaLM-2, Vicuna-13b). To make our study more com
such as Claude. Specifically, we also annotate pairwi
4 models’. We term this a more complete version of t
process are provided in Appendix I. Since answers t
annotators achieve a fair agreement.
SummEval (Fabbri et al., 2020) is a benchmark e
over 100 news articles from CNN/Daily Mail datase
four aspects: coherence, consistency, fluency, an
of the four metrics as the evaluation results.

ch (07/2024)
iewer B about your reviews of the following
iew B] {Review of reviewer B}
, and make a decision if to change your
nsupported information, core information,
nswer 1 and answer 2 by outputting the number
e other than the number in this last line."
he third turn. Similar to the review template, we
attention to. All texts above are chat history which
Right before the start of the discussion, the system
de explicit aspects to focus on.
et al. (2021))’s annotation guideline OpenAI (2022).
upported information: detecting information with no
is aspect is most important and often determines the
question has actually been answered; (3) Coherence:
the overall preference is finally determined. Moreover,
sets we test in experiments.
et al., 2023) and Vicuna80, with human annotations
ween our evaluation methods and human judgments.
ions across seven domains (e.g., economics, history,
GPT3 or Human) for each. Similar to ELI5 (Fan
1) questions from Reddit forums “r/explainlikeimfive”
evel annotations of which answer is better (overall
ded questions, which spans 9 categories and covers
email writing, math problems, etc. In the QLoRA
wise comparison scores (overall preference) across 7
ch correspond to tie, model 1 wins, and model 2 wins
ns of 4 models’ answers (i.e., GPT4, ChatGPT-3.5.,
ehensive, we add recent proprietary language models
comparisons between Claude’s answers and the other
dataset Vicuna80. More details about the annotation
pen-ended questions are even harder to compare, the
uating summarizations. It contains 1600 summaries
Hermann et al., 2015). Each summary is evaluated in
relevance. In our experiments, we take the average
...
8.18% GPT-4
5.31% 1000 Human
0% 0 100 200 300 400 500 600 700 800 900 1000 1100 1200 1300 1400 1500 1600 1700 1800 1900 2000 2100 2200 2300 2400 2500 2600 2700 2800 2900 3000 3100 3200
Figure 3: Peer rank final round weights of Match No.
each LLM reviewer. GPT-4 and Claude take Figure 4: GPT-4 Elo scores every 100 battles on the Vi48.8% and 37.7% weights. Bard got close to cuna80. Elo scores provided by the GPT-4 reviewer are conzero weights. sistently higher than human ratings, while our All (weighted)
...
GPT-3.5 0.387 0.621
human reviews at an example level, disClaude 0.319 0.607
...
Published in Transactions on Machine Learning Res
Figure 5: Pairwise win rate heatmaps: Fraction o
columns). Left: GPT-4 evaluator; Middle: our meth
rate. All results in the three sub-figures are generat
In Figure 4, we draw the line chart of how the GP
Elo algorithm. GPT-4’s score takes off as battle nu
self-enhancement across the entire process, while ou
comparisons well.
In Figure 5, we present the detailed pairwise win rat
our evaluation with GPT-4 based evaluation, as well
is based on user queries and their corresponding p
that although both approaches favor GPT-4 and Cl
All (weighted) correlate better with Arena win rate
heatmaps are in Appendix H.
4.3 Results for Peer Discussions (PD)
Prompt for Discussion By
preliminary study, we find that GPT-4 init score
the template asking for explicit Claude init score
aspects, such as core informa- Generic prompt
tion, unsupported information, w/ explicit crite
and coherence, can substan- w/ role
tially help LLM reviewers gen- w/ role & explic
...
good prompt for facilitating LLM peer discussions.
Accuracy (PDA) of GPT-4 and Claude’s initial pairw
a moderate agreement with human preference, wit
evaluators, we report three types. By “GPT-4 lead”,
opinions; by “random”, we refer to discussions wher
In discussions (the last three rows), when we use a
the discussion’s final preference PDA is around 0.69,
than GPT-4’s. When we add more explicit aspect
improvement). When we add the role/identity inform
3https://lmsys.org/blog/2023-05-25-leaderboard/
4We select aspects from WebGPT annotation guidelines m

ch (07/2024)
Model A Wins for all A vs. B Battles (A: rows, B:
All (weighted); Right: Chatbot Arena pairwise win
eparately using the same data.
4 Elo score changes as more battles are fed to the
ber increases. We can observe that GPT-4 displays
R-based evaluation correlates with human pairwise
between every two contestants (LLMs). We compare
the Chatbot Arena leaderboard. The Arena ranking3
rences for two responses. The figure demonstrates
e answers, the win rates calculated by our approach
pecially on weaker models. More pairwise win rate
GPT-4 lead Claude lead Random
- - 0.729 (±0.014)
- - 0.671 (±0.025)
0.714 (±0.018) 0.671 (±0.022) 0.686 (±0.022)
0.729 (±0.014) 0.721 (±0.018) 0.720 (±0.014)
0.743 (±0.011) 0.729 (±0.018) 0.729 (±0.011)
riteria 0.750 (±0.014) 0.721 (±0.011) 0.743 (±0.011)
mpting’s effect on Peer Discussion Accuracy (on the
rst two rows are the results before discussions (from
spectively). The last three rows are results after
e first two rows in Table 6 lists the Peer Discussion
comparison preference before discussions. They have
PT-4 leading around 5%. For the discussion-based
refer to the discussions where GPT-4 first expresses
he leader is randomly picked.
eric prompt (such as “pick your preferred answer”),
gher than Claude’s initial judgment’s PDA but lower
to the prompt 4, the PDA boosts significantly (4%
ion (Appendix G) to each turn’s prompt (“w/ role”)
oned in the previous section.
...
differ by less than 100 in Table 3), they reach stable imTable 7: Comparing peer discussions (PD) and
...
We conduct experiments on the subset of LFQA quesGPT-3
...
different reviewers on LFQA. For all LLM reGPT-3 answers with a 13.79% higher win rate. After disviewers, we take the average accuracy of all discussing with other LLMs, all models align better with hucussions they participate in. Self-enhancement
...
Justin Chih-Yao Chen, Swarnadeep Saha, and Mohit Bansal. Reconcile: Round-table conference improves reasoning via consensus among diverse llms. ArXiv, abs/2309.13007, 2023. URL https:
...
review. 1995.
Kalpesh Krishna, Aurko Roy, and Mohit Iyyer. Hurdles to progress in long-form question answering. In Proceedings of the 2021 Conference of the North American Chapter of the Association for Computational Linguistics:
...
Published in Transactions on Machine Learning Res
Aman Madaan, Niket Tandon, Prakhar Gupta, Skyle
Dziri, Shrimai Prabhumoye, Yiming Yang, Shash
Hermann, Sean Welleck, Amir Yazdanbakhsh, a
self-feedback, 2023.
Reiichiro Nakano, Jacob Hilton, Suchir Balaji, Jeff
Shantanu Jain, Vineet Kosaraju, William Saunde
with human feedback. arXiv preprint arXiv:2112.
David Nicol, Avril Thomson, and Caroline Breslin. R
review perspective. Assessment & evaluation in h
OpenAI. Webgpt annotation guidelines. 20
1i0h5dorAZydNNiDJamqq_ZGSpzaPKvLcuJvFaO87l
OpenAI. Gpt-4 technical report. ArXiv, abs/2303.0
Long Ouyang, Jeffrey Wu, Xu Jiang, Diogo Almeid
Sandhini Agarwal, Katarina Slama, Alex Ray, et a
human feedback. Advances in Neural Information
Lawrence Page, Sergey Brin, Rajeev Motwani, and T
order to the web. Technical report, Stanford InfoL
Kishore Papineni, Salim Roukos, Todd Ward, and W
of machine translation. In Proceedings of the 40t
Linguistics, pp. 311–318, 2002.
Joon Sung Park, Joseph C O’Brien, Carrie J Cai,
Bernstein. Generative agents: Interactive simulacr
2023.
Toby Walsh. The peerrank method for peer asse
Conference on Artificial Intelligence, pp. 909–914,
Alex Wang, Kyunghyun Cho, and Mike Lewis. A
consistency of summaries. In Proceedings of the 58
Linguistics, pp. 5008–5020, 2020.
Peiyi Wang, Lei Li, Liang Chen, Dawei Zhu, Binghu
Large language models are not fair evaluators, 20
Yizhong Wang, Hamish Ivison, Pradeep Dasigi, Jack
Wadden, Kelsey MacMillan, Noah A. Smith, Iz B
exploring the state of instruction tuning on open
Fangyuan Xu, Yixiao Song, Mohit Iyyer, and Eunso
...
Weizhe Yuan, Richard Yuanzhe Pang, Kyunghyun C
Self-rewarding language models. ArXiv, abs/2401.1
CorpusID:267035293.
Tianyi Zhang, Varsha Kishore, Felix Wu, Kilian Q W
generation with bert. arXiv preprint arXiv:1904.0

ch (07/2024)
allinan, Luyu Gao, Sarah Wiegreffe, Uri Alon, Nouha
Gupta, Bodhisattwa Prasad Majumder, Katherine
Peter Clark. Self-refine: Iterative refinement with
u, Long Ouyang, Christina Kim, Christopher Hesse,
et al. Webgpt: Browser-assisted question-answering
32, 2021.
inking feedback practices in higher education: a peer
er education, 39(1):102–122, 2014.
URL https://docs.google.com/document/d/
/edit.
4, 2023.
Carroll Wainwright, Pamela Mishkin, Chong Zhang,
Training language models to follow instructions with
ocessing Systems, 35:27730–27744, 2022.
Winograd. The pagerank citation ranking: Bringing
, 1999.
Jing Zhu. Bleu: a method for automatic evaluation
nnual meeting of the Association for Computational
eredith Ringel Morris, Percy Liang, and Michael S
f human behavior. arXiv preprint arXiv:2304.03442,
ment. In Proceedings of the Twenty-first European
14.
ng and answering questions to evaluate the factual
Annual Meeting of the Association for Computational
in, Yunbo Cao, Qi Liu, Tianyu Liu, and Zhifang Sui.
essel, Tushar Khot, Khyathi Raghavi Chandu, David
agy, and Hanna Hajishirzi. How far can camels go?
ources. ArXiv, abs/2306.04751, 2023b.
hoi. A critical evaluation of evaluations for long-form
nefits of peer review on students’ writing. Psychology
Sainbayar Sukhbaatar, Jing Xu, and Jason Weston.
0, 2024. URL https://api.semanticscholar.org/
nberger, and Yoav Artzi. Bertscore: Evaluating text
75, 2019.
...
Published in Transactions on Machine Learning Res
Figure 9: The form used by human annotators for in
response on several metrics from 0 to 10. Explanati
and next button navigate between responses.
I Human Annotation for Pairwise Prefer
Since completing one HIT can take a considerable am
saving their work at any stage in the middle of the
representation of the current responses, which may
We annotate part of the pairwise comparisons of m
The screenshot is as illustrated in Figure 10.

ch (07/2024)
dual rating of a model. Sliders are included to rate a
can be entered in the text area below. The tab bar
ce
nt of time (6-10 min), we added a button that allows
T. This button populates a text area with a JSON
copied into a file.
l answers on Vicuna80. We built an interface form.
...
In this section, there are four examples showing opinion-altering (OA), opinion-holding (OH), and postagreement opinion-altering. In the following discussions, all texts before a colored reviewer’s name are
...
stock funds, see [*Ancient Kanesh: A Merchant Colony in Bronze Age Anatolia*](https://books.google.com/books?id=qXGgCgAAQBAJ) by Mogens Larsen.
[The End of Answer 1]
...
companies operated, who invested in them, how long they lasted, and what purposes they served. The answer is coherent and flows logically from one idea to the next.
...
[The End of Answer 2]
[The Start of Reviewer 1’s Evaluation]
Answer 1 provides a more detailed and informative response to the question. It suggests that Frederick Douglass could swim, based on his own accounts of his childhood.
27

Published in Transactions on Machine Learning Research (07/2024)
The answer also provides some context about the conditions of enslaved children and
their interactions with white children, which could have included learning to swim together. This answer is coherent and seems to be supported by the information provided.
...
1 or 2 respectively. Do not output anything else other than the number in this last line.
GPT-4:
After considering Reviewer 2’s evaluation, I still stand by my initial preference for Answer 1. While it’s true that neither answer provides
...
Source https://www.sciencefocus.com/the-human-body/why-do-men-go-bald-fromthe-top- of-their-heads/
[The End of Answer 2]
[The Start of Reviewer 1’s Evaluation]
Here is my evaluation of the two answers:
...
supply to the hair follicles, causing them to shrink and eventually disappear. This answer also provides a source for its information, which adds credibility to its explanation.
...
Depth and exploration of alternative perspectives are valuable in building understanding. Answer 2 acknowledges the role of DHT while proposing that increased tension
