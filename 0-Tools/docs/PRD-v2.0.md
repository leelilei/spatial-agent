...
- **V2.0-M1 指挥舱** ✅：概览落地页 + 取消 Next Work tab（R4, R5）。
- **V2.0-M2 数据模型** ✅：`project.yaml` 双读兼容 + `migrate.py`（R1, R2, R3）。
  合规检查接受 `project.yaml` 满足 L1/L2；SMGA 已迁移为单文件 V2 作示范。
- **V2.0-M3 牵引与收尾** ✅：升级提示（概览）、决策/产出下钻页、`check --doctor`（R7, R8, R9）。
  下钻页现为 6 个：概览 / 路线图 / 进展 / 论文 / 决策 / 产出（决策、产出仅在有内容时出现）。
