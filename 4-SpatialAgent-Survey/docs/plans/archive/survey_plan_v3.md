...
| Embodied AI / VLN surveys (Anderson 2018, Gu 2022) | 导航与任务完成 | 聚焦社会行为，而非导航 |
| LLM Game Agent surveys (Hu 2024) | Agent 架构与游戏能力 | 聚焦空间维度，而非通用架构 |
| Multi-agent simulation surveys (Gao 2023) | 社会结构涌现 | 聚焦空间对社会结构的调制 |
| Spatial reasoning benchmarks (Li 2024, Yamada 2024) | LLM 空间推理测试 | 聚焦长时程社会模拟，而非单次推理 |

互补关系声明（写入 §1）：本文与 Feng et al. 互补——Feng 回答"LLM 能否理解空间"（能力），本文问"如果能，这对社会行为意味着什么"（效应与议程）。
...
| 1.4 | 设计 evidence table 模板（按 §2.7 编码方案）+ 撰写 coding manual | `evidence_table_template.md` + `coding_manual.md` |
...
| 周次 | 任务 |
|------|------|
| Week 10 | 全文通读，检查论证一致性；确认所有 claim 有 evidence table 支撑 |
