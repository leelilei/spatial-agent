#!/bin/bash
# Resume-safe Yunwu calibration for Wave-3 public variant v0.
set -u
...
run_model() {
  name="$1"
  config="$2"
  output="$RESULT_ROOT/native_wave3_public_v0_2x8x1_${RUN_DATE}_${name}"
  for attempt in $(seq 1 100); do
    if [ -f "$output/run_manifest.json" ] && [ "$(jq -r '.status' "$output/run_manifest.json")" = "complete" ]; then
      echo "v0 $name COMPLETE"
      return 0
    fi
    echo "v0 $name attempt $attempt"
    resume_flag=
