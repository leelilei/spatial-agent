#!/bin/bash
# Resume-safe Apilio calibration for Wave-3 public variant v2.
set -u
...
SCENARIOS=ci11w3_comp_harbor_grid_v1_v2,ci11w3_disr_suburb_polycentric_v1_v2,ci11w3_memo_harbor_grid_v1_v2,ci11w3_mult_suburb_polycentric_v1_v2,ci11w3_poi__suburb_polycentric_v1_v2,ci11w3_reso_metro_radial_v1_v2,ci11w3_soci_metro_radial_v1_v2,ci11w3_time_harbor_grid_v1_v2
RUN_DATE=2026-08-06
...
run_model() {
  name="$1"
  config="$2"
  output="$RESULT_ROOT/native_wave3_public_v2_apilio_2x8x1_${RUN_DATE}_${name}"
  for attempt in $(seq 1 100); do
    if [ -f "$output/run_manifest.json" ] && [ "$(jq -r '.status' "$output/run_manifest.json")" = "complete" ]; then
      echo "v2 apilio $name COMPLETE"
      return 0
    fi
    echo "v2 apilio $name attempt $attempt"
    resume_flag=
  ...
  echo "v2 apilio $name INCOMPLETE"
  return 1
}
