...
SCENARIOS=ci11w3_reso_suburb_polycentric_v1_v0,ci11w3_reso_harbor_grid_v1_v1,ci11w3_reso_metro_radial_v1_v2
...
run_model() {
  name="$1"
  config="$2"
  output="$RESULT_ROOT/native_wave3_resource_final_2x3x1_2026-08-06_${name}"
  resume_flag=
  [ -f "$output/traces.json" ] && resume_flag=--resume
  python "$RUNNER" --benchmark-config "$BENCHMARK" --agents "$AGENTS" \
    --llm-config "$config" --scenario-ids "$SCENARIOS" --results-dir "$output" $resume_flag
}
