#!/bin/bash
# Resume-safe Apilio rerun for the three locally hardened Wave-3 v1 items.
set -u
...
SCENARIOS=ci11w3_comp_suburb_polycentric_v1_v1,ci11w3_reso_harbor_grid_v1_v1,ci11w3_time_suburb_polycentric_v1_v1
RUN_DATE=2026-08-06
...
run_model() {
  name="$1"
  config="$2"
  output="$RESULT_ROOT/native_wave3_public_v1_hardened_2x3x1_${RUN_DATE}_${name}"
  for attempt in $(seq 1 100); do
    if [ -f "$output/run_manifest.json" ] && [ "$(jq -r '.status' "$output/run_manifest.json")" = "complete" ]; then
      echo "v1 hardened $name COMPLETE"
      return 0
    fi
    echo "v1 hardened $name attempt $attempt"
    resume_flag=
  ...
  echo "v1 hardened $name INCOMPLETE"
  return 1
}
