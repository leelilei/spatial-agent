...
| 社会互动质量 | SOTOPIA、AgentSense | 对话能够较合理地追求私有社会目标 | 物理行动和声称的结果真实发生 |
| 移动真实性 | MobiSim-Bench、When Plausible Is Not Realistic | 生成群体在部分指标上接近人类移动 | 某个 Agent 持续维护并完成了自己的意图 |
| 可执行结果 | ChinaTravel、tau-bench、AppWorld | 计划或最终状态满足明确谓词 | 中间城市轨迹连续、可信并进行了合理重规划 |

因此，CityAgency 的可辩护缺口不是“以前没人测过城市 Agent”，而是：

> 现有工作分别测试城市推理、社会目标、经验移动分布或可执行最终状态。CityAgency
> 测试 Agent 能否将一个合理的私有计划执行为连续且环境有效的城市轨迹，并由状态变化
> 提供目标完成证据。
...
|---|---|---|
| 目标证据 | tau-bench、AppWorld | 对位置、库存、预算、消息、共处和截止时间执行确定性谓词 |
...
| 软性可信度 | SOTOPIA、GATSim | LLM/人类 Judge 只评价可信度和社会质量，不负责最终完成裁定 |
