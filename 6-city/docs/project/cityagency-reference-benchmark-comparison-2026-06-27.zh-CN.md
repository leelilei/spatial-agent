...
|---|---|---|---|---|---|---|
| **CityAgency（ours）: Plausible Plans, Impossible Traces** | LLM 城市 Agent 能否把合理的私有计划执行为连续、可行、可信且可由环境证明完成的城市轨迹 | 已接入固定版本的 GATSim、SOTOPIA-style `LLMAgent`、Generative Agents、AgentSociety 决策层，统一 typed executor；这里的 SOTOPIA 指其 benchmark lineage 中的 `LLMAgent` 决策 policy，不是把 SOTOPIA benchmark 本身当作 agent；以 `gpt-5.4-mini` 为主模型，并用 `gpt-5.4` 做配对模型敏感性测试；另有受控 Utility、Direct、Plan-then-Act、Reactive 基线 | 在可重置微型城市中设置私有目标、时间窗、预算、POI、社会关系和中途扰动；环境拥有权威状态并记录 typed evidence；已完成 4 架构 × 4 压力场景 × 3 次重复、4 架构 × 12 场景广度、16 个跨模型配对单元、网络/设施/社交事件的 12 个 control-treatment 配对单元、设施关闭的 3 架构 × 3 次重复，以及 4 架构 × 6 个社交结果场景 × 3 次重复 | Task Completion、Trace Feasibility、Impossible Trace Rate、Constraint Satisfaction、Replanning Success、False Continue、Face/Trace Plausibility Gap、`pass^k`、条件恢复率、跨 Judge 与跨模型敏感性 | 226 条存档记录、210 次独立真实 API 执行中没有单一架构全面领先；跨模型实验出现架构排序翻转；设施关闭重复实验中 GATSim 保持 3/3 joint recovery，而 Generative Agents 与 AgentSociety 均为 0/3。社交结果家族中，GATSim 完成 15/21 个共处结果、AgentSociety 4/21、Generative Agents 2/21、SOTOPIA-style `LLMAgent` 为 0/21；但该 adapter 仍有 61.1% 的轨迹完全可行，形成跨六类机制的“合法但无效”证据。软可信度与硬执行结果持续解耦，跨 Judge 一致性仅中等；最终冻结仍需双人盲标 | 为使用 LLM Agent 研究交通、活动、设施可达性、灾害响应和政策行为提供微观有效性检查，避免把语言流畅但不可执行，或合法但未完成目标的合成轨迹当作城市证据 |
