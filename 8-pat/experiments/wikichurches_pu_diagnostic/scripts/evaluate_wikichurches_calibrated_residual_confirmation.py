#!/usr/bin/env python3
"""Evaluate frozen calibrated-residual fusion on the test split once."""
...
def main() -> None:
    ...
    official_stats = frozen["official_calibration"]
    official_evidence = residual_evidence(
        local["official_local_scores"][ratio_key].float(),
        torch.tensor(official_stats["negative_mean"]),
        torch.tensor(official_stats["negative_scale"]),
    )
    random_evidence = []
    for scores, stats in zip(
        local["random_local_scores"][ratio_key],
        frozen["random_calibration"],
        strict=True,
    ):
        random_evidence.append(
            residual_evidence(
                scores.float(),
                torch.tensor(stats["negative_mean"]),
                torch.tensor(stats["negative_scale"]),
            )
        )
    random_evidence = torch.stack(random_evidence)
    ...
    lines = [
        "# WikiChurches calibrated-residual frozen confirmation",
        "",
