#!/usr/bin/env python3
"""Run and gate the PAT-D-260728-007 K2 subset-selection screen."""
...
from cub_subset_selection import STRATEGIES
...
def main():
    ...
    if any(
        ...
        raise RuntimeError("PAT-D-260728-007 may decode official train only")
    ...
    if complete:
        ...
        for strategy, prediction in predictions.items():
            ...
            results[strategy] = {
                ...
                "delta_pp_vs_random_k2_mean": 100
                * (
                    ba
                    - protocol["dependencies"]["random_k2_mean_oof_ba"]
                ),
                "negative_transfer_class_rate_vs_global": negative_rate,
                "negative_transfer_rate_change_pp_vs_random_k2_mean": 100
                * (
                    negative_rate
                    - protocol["dependencies"][
                        "random_k2_mean_negative_transfer_class_rate"
                    ]
                ),
        ...
        random_metrics = []
        for seed in protocol["controls"]["random_reference_selection_seeds"]:
            prediction = random_reference[f"K2_S{seed}_predictions"]
        ...
        summary.update(
            {
                ...
                "random_k2_reference": random_metrics,
                "results": results,
                "overall_gate_pass": winner is not None,
                "winner": winner,
                "winner_branch": branch,
                "next_stage": (
                    "NEWLY_FREEZE_PAIRED_CONFIRMATION"
                    if winner is not None
                    else "STOP_ACTIVE_SELECTOR_DIRECTION"
                ),
            }
        )
    else:
    ...
    np.savez_compressed(
        args.output_dir / "cub_gradient_coverage_predictions.npz",
        labels=labels,
    ...
    (args.output_dir / "cub_gradient_coverage_summary.json").write_text(
