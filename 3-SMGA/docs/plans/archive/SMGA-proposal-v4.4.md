...
> Version: **0.4.4**
> Date: **2026-05-19**
> Status: v4.4 update: claim recalibrated from a narrow memory-contract story to a broader long-horizon memory-reliability story
> Relation to v4.3: keeps the AGA-style paper shape, two-module method, main ablation table, and mechanism analysis, but sharpens the research claim: SMGA is not merely a schema/field-engineering proposal; it studies the reliability bottleneck of turning past social experience into auditable, contradiction-aware, planning-actionable context that changes future behavior.

---

## 0. One-Line Version

SMGA studies the **memory-reliability bottleneck** in long-horizon GA-style agents: not whether agents can store or retrieve memories, but whether past social experience can be converted into **auditable, contradiction-aware, planning-actionable context** that actually changes future behavior.

> **SMGA moves GA-style social memory from retrievable reflection to reliable memory-to-action context: typed, entity-grounded, evidence-verified, contradiction-aware, validity-scoped, and exposed to planning through auditable affordances.**

The central question:

> Under matched model, prompt, compute, interface, and scenario controls, does adding an explicit social-memory-to-planning contract improve history-dependent planning and behavior beyond GA reflection, prompted reflection, placebo memory, and graph-memory baselines?

---

## 1. What Changed in v4.4

v4.4 keeps the v4.3 paper shape but recalibrates the claim. The v4.3 wording risked making SMGA sound like a narrow schema-contract paper, or like it assumed GA cannot use memory for planning. v4.4 instead frames the project around **long-horizon memory reliability**: GA already has memory, retrieval, reflection, and planning; the open question is whether a more explicit, auditable, contradiction-aware memory-to-action interface makes social experience more reliably useful for future behavior.

| v4.3 risk | v4.4 change |
|---|---|
| “GA cannot reliably turn social experience into planning context” sounded too strong as a background claim | Rewritten as an empirical question: GA lacks an explicit memory-to-planning contract; SMGA tests whether that contract is load-bearing |
| The claim could sound like field/schema engineering | Reframed as a **memory-reliability** problem for long-horizon agents: can past experience become auditable, scoped, contradiction-aware action context? |
| AGA comparison made SMGA look smaller because AGA has the very broad “reduce cost” claim | Clarified the parallel: AGA addresses the **cost bottleneck**; SMGA addresses the **memory-reliability bottleneck** |
| The proposal could imply GA has no useful memory | Added explicit language that GA memory streams and reflections can support believable behavior; SMGA builds on them rather than replacing them |
| Method fields risked looking ornamental | Reconnected fields to reliability functions: evidence IDs enable audit, contradiction IDs prevent one-sided memory use, validity scope prevents overgeneralization, planning affordances expose action implications |

### v4.3 changes retained

v4.3 changed the **paper shape**. Inspired by the clarity of *Affordable Generative Agents* (AGA), v4.3 rewrote SMGA around one bottleneck, two modules, and a clean main ablation story. v4.4 keeps this discipline, but uses a more careful bottleneck statement:

```text
AGA:  GA has a cost bottleneck.
      Reduce repeated LLM inference while preserving believable behavior.

SMGA: GA-style long-horizon agents face a memory-reliability bottleneck.
      Test whether structured, evidence-grounded social memory makes past experience
      reliably useful for future planning and behavior.
```

| v4.3 change retained | Why it stays in v4.4 |
|---|---|
| Two modules: **Structured Social Memory Formation** and **Evidence-Grounded Planning Interface** | Keeps the method easy to understand before the low-level L1–L4 implementation details |
| AGA-style main ablation table: `M0_GA`, `M0_prompted`, `M2_memory_only`, `M3_placebo`, `M3_actionable`, `GraphMemory_social_schema` | Makes the causal story testable: representation, interface, placebo, prompt, and graph-memory controls |
| Mandatory calls/tokens/latency/cost-normalized reporting | Prevents the “SMGA just thinks more” objection |
| Mechanism analysis: error decomposition, field ablation, history-dependence taxonomy | Explains when and why structured memory helps, without making every diagnostic a headline claim |
| AGA related-work distinction | Avoids confusion with AGA’s Social Memory, which is a cost-saving compression module rather than an auditable planning-memory contract |
...
> **SMGA addresses the memory-reliability bottleneck in long-horizon generative agents: whether past social experience can be converted into auditable, contradiction-aware, planning-actionable context that actually changes future behavior.**

The claim is not that GA-style agents lack memory. GA already has memory streams, retrieval, reflection, and planning. The claim is that standard GA-style reflection does not impose an explicit **memory-to-planning contract**: a social memory is not required to be typed, entity-grounded, provenance-checkable, contradiction-aware, validity-scoped, or exposed to the planner through auditable affordances.

SMGA tests whether adding this contract improves history-dependent planning and behavior beyond matched memory baselines.

The main contrast is:

```text
GA-style memory stream + retrieval + reflection + planning
vs.
GA-style memory stream + structured social memory formation + evidence-grounded planning interface
```

A more compact paper claim:

> Long-horizon agents do not only need to remember more; they need memories that are reliable enough to guide action. SMGA tests whether structured, evidence-grounded social memory makes this memory-to-action link more reliable.

Explicitly **not** claimed:

```text
GA lacks memory.
GA cannot ever use past experience for planning.
...
Every SMGA field is assumed useful before ablation.
```

The contribution is falsifiable: if prompted reflection, compute-matched reflection, placebo memory, or graph memory matches SMGA, the claim is downgraded according to §16.
...
AGA showed that GA can be optimized cleanly by identifying a concrete bottleneck and modifying only the relevant interaction pathways. SMGA follows that style, but targets a different bottleneck: not repeated LLM cost, but the reliability of the link from social experience to future action. The problem is not that GA has no memory; the problem is that standard GA-style memory and reflection do not guarantee auditable, contradiction-aware, validity-scoped planning context.
...
v4.4 keeps five research questions, but maps the hypotheses into three claim families so the paper is not framed as five unrelated claims. The dropped RQ from v3 (cost/model dependence) remains a reporting requirement rather than a primary question.

**RQ1 — Memory reliability.** Does SMGA produce more accurate, evidence-supported, contradiction-aware structured social memories than GA reflection, prompted GA reflection, and graph memory with social schema?

**RQ2 — Schema contribution.** Does the proposed schema matter, or do coarse, emergent, or oracle schemas match it?

**RQ3 — Planning transfer.** Do structured social memories become reliable planning context, improving planning decisions beyond what recall and abstraction quality alone predict?
...
v4.4 does not present five hypotheses as five parallel paper claims. Instead, the experiments test a staged memory-reliability mechanism:

```text
social experience
→ auditable, contradiction-aware social memory
→ memory-grounded planning
→ history-dependent behavior
→ external lifelong-social validation
```
...
### Claim Family 1 — Reliable evidence-grounded social memory

**Core claim.** SMGA produces more accurate, human-verifiable, evidence-supported, contradiction-aware structured social memories than prompted reflection and graph-memory baselines.
...
**Interpretation.** H1a asks whether the right social information is recalled with valid evidence. H1b asks whether the system turns episodic facts into valid social abstractions. Together, they support the memory-reliability representation claim; alone, they do not prove behavioral improvement.

### Claim Family 2 — Memory-grounded planning and behavioral transfer

**Core claim.** SMGA improves history-dependent planning and behavior by making social memory usable as grounded action context, and the improvement is driven by memory content rather than interface, prompt, or compute effects.
...
v4.4 presents SMGA as two modules rather than starting from four low-level layers. The four layers remain, but they are now grouped by the paper's main causal story: making social memory reliable enough to guide action.
...
**Affordable Generative Agents (AGA).** AGA optimizes GA by reducing the cost of repeated agent-environment and inter-agent interactions through Lifestyle Policy and Social Memory. SMGA is orthogonal. It does not aim to reduce cost; it aims to improve the reliability of the memory-to-action link in long-horizon social agents. AGA compresses and reuses interaction patterns to preserve believability at lower token cost. SMGA verifies, scopes, tracks contradictions, and operationalizes social memories for planning, with cost reported as a confound rather than a headline claim.
...
> A staged, controlled architecture-and-evaluation study of whether two SMGA modules — structured social memory formation and an evidence-grounded planning interface — improve the **reliability of the memory-to-action link** in long-horizon GA-style agents.
...
```text
Claim Family 1:
M3 > M0_prompted and M3 > GraphMemory_social_schema on reliable evidence-grounded memory quality
...
If only Claim Family 1 holds, the paper is a structured-memory reliability and audit paper. If Claim Families 1 and 2 hold, it is a strong SMGA architecture paper: structured social memory improves not only memory quality but also memory-grounded behavior. If all three claim families hold, the paper claims SOTA on history-dependent social memory under the evaluated benchmark and baseline set — and nothing broader.
